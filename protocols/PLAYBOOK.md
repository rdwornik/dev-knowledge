...
- [The harness — definition and closure per release](#the-harness--definition-and-closure-per-release)
  - [The definition](#the-definition)
  - [Closure per release](#closure-per-release)
  - [Universalization "done"](#universalization-done)
...
**Codex reviewer config:** The global reviewer config (role, checklist, output format) lives at `~/.codex/AGENTS.md`; canonical source tracked in `.dev-knowledge/deploy/global-instructions-codex.md` (ADR-54). Per-repo `AGENTS.md` adds only repo-specific rules — it does not repeat the global config.
...
## The harness — definition and closure per release
<!-- scope: meta -->

**Adopted verbatim, not restated.** The two blocks below are pasted from
`to-browser/HANDOVER-ARCHITECTURE-2026-09-08.md` §0 and §3, on ruling R-P6 of
`to-cc/DECLARE-SESSION-PLAN-2026-09-08.md`, which gives this section as their home and
resolves that file's `OPEN` carrier to it. A definition that is paraphrased on landing is a
second definition; these are the outgoing seat's own words, and the reason they were owed a
home is that the word "harness" had been in operator use for months with no sentence behind
it anywhere in the corpus.

### The definition

> The harness is the machinery that turns an intent into a **landed, verified, remembered
> change across N repositories, with the human at exactly five points** (GO · ratification ·
> tag · destructive acts · seat release). Its closure per release is measurable: shape
> parity, orphans, ship-gate, human points on the critical path, tokens per window,
> unattended completion rate. "Global, universal, dynamic" means: one implementation
> (global), identical shape everywhere so that one implementation suffices (universal), facts
> derived from data at commit time so the repo is never wrong at rest (dynamic).

### Closure per release

The six closure measures are named in the definition itself and are not re-listed here — a
second copy of a list is the drift this whole section exists to prevent. What the definition
does **not** say, and what a release therefore has to state for itself, is which of the six
are instrumented at that point and which are still folklore. A window's close prints the
measures it can compute and names the rest as uninstrumented; a measure reported from memory
is not reported.

Two consequences follow directly from the definition and are worth stating where a seat
reads them:

- **Five points is the budget, not the observation.** A window that consumed more operator
  points than five did not break the definition — it measured the gap. A plan is graded by
  how many points it removes.
- **An organ with no trigger is not part of the harness.** "Machinery that turns an intent
  into a landed change" excludes a capability nothing fires. That is the predicate the orphan
  work reads, and it is why a capability nobody is required to use has to be *declared*
  optional rather than left to be rediscovered.

### Universalization "done"

> Universalization "done" = corp-monorepo passes the seal with zero waivers-without-reason,
> its graph report has zero orphans, and the operator opens it and works.

The third clause is not decoration. The first two are machine-checkable and the third is not,
which is deliberate: a fleet that passes every gate and that the operator cannot comfortably
open has satisfied the mechanism and missed the point.
