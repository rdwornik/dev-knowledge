...
- **Why:** Two bugs witnessed during 2026-05-14 session via parallel ai-council handoff generation:
  - **Bug 1** — articulation gate item #1 hardcodes *"what is `.dev-knowledge`'s function in the ecosystem (Scrum Master / methodology guardian / auditor)?"* with no `{TARGET_REPO}` substitution (the `{repo}` substitution mechanism exists elsewhere in the template but was not extended to this line). Ai-council Stage 3 reproduces the literal text, misorienting the new chat.
  - **Bug 2** — `02_VISION.md` generation is unconditional `cp .dev-knowledge/VISION.md → 02_VISION.md` per template spec. Cross-repo bundles ship without target repo's own mission statement, breaking ADR-33 (every repo has own `VISION.md`).
- **Captured here, not fixed here.** Per operator decision 2026-05-14 (Option B after architect's verification pushback on four errors in the original briefing-note proposal — Hard Constraint #3 missed, stale directive #2 state assumed, fabricated "ADR-29 atomic convention", circular citation). v3.3.2 = next dedicated `.dev-knowledge` session's **primary objective**.
