...
- [#105] [P2][S] PreToolUse immutability guard — block Edit on `docs/decisions/*`, transcripts, handoffs, audits (new-file creation allowed); extends the block-onedrive pattern; immutable paths become a second zone class under the ADR-75 register (amending ADR) · Done when: an in-place edit attempt on an existing ADR is blocked fail-closed and a new-ADR creation passes
- [#107] [P3][S] Worktree convention for parallel sessions — PLAYBOOK rule: 2+ concurrent CC sessions on one repo = separate git worktrees; canonical fix for the witnessed parallel-session tree-contamination gotcha · Done when: the rule lands in PLAYBOOK · refs ADR-61
...
- [#101] [P2][S] SessionStart billing-leak sentinel — fail-soft check: if `ANTHROPIC_API_KEY` is visible inside the session, print a one-line WARN (shim may be clobbered — npm updates can overwrite `AppData\Roaming\npm\claude.cmd`) · Done when: the check fires in a deliberately-leaked test session and stays silent in a clean one · refs billing-trap LESSONS 2026-06-06, `surface-closures.ps1` pattern
...
- [#106] [P3][S] Auto Mode evaluation — classified permission middle-ground between bypass and babysitting (3 tiers, injection probe, audit trail) · verify-first: current availability/state on the Max plan · Done when: a go/no-go posture decision is recorded · refs unattended-run posture, ADR-68 autonomy boundary
- [#108] [P3][S] Projects-as-browser-context-layer evaluation — could claude.ai Projects carry the stable bundle layers (01_ROLE/02_METHODOLOGY) for browser chats; named counterweight: Project files live outside the repo, unversioned — the exact ownership-cadence drift E1 warns about · Done when: a go/no-go decision is recorded · refs HANDOFF_PROCESS, #25
