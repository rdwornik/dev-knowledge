...
- [#88] [P3][M] Evaluate graphify (github.com/safishamsi/graphify) for fleet adoption — project-scoped pilot (`--project`) in corp-monorepo ONLY, gated on a pre-pilot security review of its PreToolUse hook code; measure (a) graph quality vs the hand-curated codemap, (b) token cost of 3 representative navigation questions graph-query vs grep baseline, (c) maintenance overhead (rebuild hook vs hub-governed hook set) · Done when: the pilot measurements + an adopt/reject ruling are recorded — pre-registered kill criterion: adopt ONLY if token savings or navigation quality are material AND maintenance fits the hub governance model; sequence after #84 codification · refs #84, #80, ADR-70, ADR-71
...
- [#87] [P3][S] File a /bug report to Anthropic — per-agent model routing non-functional in Dynamic Workflows on CC 2.1.162 (all workflow subagents run Haiku 4.5 regardless of the `model` arg), with the closeout probe table (run wf_018a70d0) as evidence · Done when: the report is filed · refs #83, LESSONS 2026-06-04 (routing probe)
