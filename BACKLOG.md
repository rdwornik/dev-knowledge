...
- [#83] [P3][S] Harden `validate_backlog` to reject struck-through or in-place "RESOLVED/DONE" task lines (the schema gap pilot finding F2 exposed: the hook passed the in-place [#79] stub that violated ADR-65 done-items-leave) · Done when: a struck-through / in-place-resolved task line fails `validate_backlog` with a test fixture · refs ADR-65, ADR-66, docs/audits/2026-06-04-pilot81-hub-conformance-digest.md F2; closeout note CORRECTED 2026-06-05 — the earlier "per-agent routing non-functional, Haiku-only de-facto" reading (probe wf_018a70d0) was refuted by the Phase-C3 re-probe: the cause was our own `CLAUDE_CODE_SUBAGENT_MODEL=haiku` override (now removed), not the platform; with pins honored on BOTH the Agent-tool and workflow-engine paths, the Sonnet/Haiku fan-out in #81/#82's design holds. The core hardening task here is unaffected. · evidence (nightly 2026-06-05 force-run digest, H1): #79 initial closure violated ADR-65 done-items-leave — in-place strikethrough + RESOLVED marker in commit `052e311`, corrected by `fb24810`; concrete fixture-source for this hardening
