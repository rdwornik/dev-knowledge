...
- [#73] [P2][L] Bundle Tier-1 (Stop-hook propose-closures + ruff gate + /review-closures + SessionStart surfacing) as one methodology plugin and install it across all registered ecosystem repos — **5a done**: plugin packaged at `plugins/tier1-lifecycle/` (CC-standard; marketplace at `.claude-plugin/`) + piloted/verified end-to-end on ai-council; lesson-promotion dropped from scope (deferred, see Unit-6 JOURNAL); remaining → 5b converge `.dev-knowledge` itself onto the plugin + roll out to the other registered repos · Done when: a single plugin install propagates the Tier-1 lifecycle to every registered repo with no per-repo re-derivation · refs ADR-70 (Tier 1)
