...
- [#85] [P3][L] Night-run architecture — LOCAL track (direction ratified 2026-06-06: local = Tier-2 cross-repo host, structural advantage: only local sessions see sibling repos, ADR-74; mechanism = native Desktop scheduler behind an n=1 gate; rider R1: gate run must verify Desktop scheduler exists in operator's app version on Windows, per-task permissions/model/working-folder config, catch-up = ONE run for the most recent miss, lid-closed sleep skips; rider R2: missed nights tolerated by design — fail-soft + catch-up + SessionStart surfacing is the accepted degradation path, no alerting; natural n=1 workload: scheduled fleet-baseline run as a scheduled local task; fire-and-complete stages, Workflow tool pre-allowed in settings, session Write/Edit deny + post-run fleet `git status --porcelain` tripwire, digest-only output) · Done when: a scheduled local night-run executes a read-only conformance/review workflow end-to-end and lands a morning digest · refs #82 (profiles), #84 (codification), ADR-68, ADR-74, docs/audits/2026-06-03-phase0-workflow-gates-findings.md (gates 4/5), docs/archive/2026-06-05-agent-automation-external-research-note.md
