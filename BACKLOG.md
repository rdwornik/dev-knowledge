...
- [#8] [P2][M] Implement the analysed hooks (session-end clean-tree/staleness + SessionStart lessons-retrieval) and resolve the review-hook overlap — session-end **propose-closures** detector landed (`scripts/propose_closures.py` + `.claude/settings.json` Stop hook, ADR-70 Tier-1); clean-tree/staleness + SessionStart lessons-retrieval + review-hook overlap still remain · Done when: the remaining hooks run + are documented · refs 2026-05-29 hooks audit, ADR-70 (Tier-1 lifecycle hooks)
