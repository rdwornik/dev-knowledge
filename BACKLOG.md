...
- [#85] [P3][L] Night-run architecture — LOCAL track (Task Scheduler + `claude -p`): fire-and-complete stages (one uninterrupted process per stage — workflow runs do not survive a CC exit, Phase-0 gate 5 = S1), Workflow tool pre-allowed in settings (else a headless launch auto-denies at the approval gate), session Write/Edit deny + post-run fleet `git status --porcelain` tripwire, digest-only output; also evaluate native Desktop scheduled-tasks tier vs OS Task Scheduler (sleep/lid constraints, catch-up semantics, per-task envelope) — refs the 2026-06-05 external research note · Done when: a scheduled local night-run executes a read-only conformance/review workflow end-to-end and lands a morning digest · refs #82 (profiles), #84 (codification), ADR-68, docs/audits/2026-06-03-phase0-workflow-gates-findings.md (gates 4/5), docs/archive/2026-06-05-agent-automation-external-research-note.md
- [#86] [P3][M] Cloud-night decision package (Routines) — operator-gated: the selective-push decision (which repos may leave the machine), the ADR-71 hooks relative-path → URL migration (a cloud runner has no local hub clone), and the R2 saved-workflow distribution ruling (committed hub orchestration vs personal-only) · Done when: each of the three sub-decisions is recorded (ADR or operator note) · refs ADR-71, ADR-70, #84, docs/audits/2026-06-04-pilot81-hub-conformance-digest.md (R2), docs/archive/2026-06-05-agent-automation-external-research-note.md (sub-decision 1 secrets-boundary criteria + sub-decision 3 ownership-cadence argument) — sub-decision 1 operator ruling 2026-06-06: SELECTIVE-PUSH = hub + corp-monorepo + ai-council are cloud candidates; corp-sca + corp-ops excluded (too small). Precondition executed: corp-monorepo pre-push sensitivity sweep (secrets CLEAN, 2026-06-06); corp-monorepo pushed to private GitHub remote 2026-06-06. — sub-decision 2 (ADR-71 hooks relative-path → cloud) RECORDED 2026-06-06: **ADR-72** (Path A self-containment) — hub is PRIVATE, so plugin/skill (the #86.3 vehicle) AND URL-fetch are all inert in a clone-only cloud session; cloud Routines are self-contained, no hub ref load-bearing, no resolver shipped. Remaining: sub-decision 3 (R2 saved-workflow distribution ruling).
