...
- [#12] [P2][M] Create the evolution .jsonl logs (or repoint hooks/protocol to evolution-log.md) + add one read-only session-close automation · Done when: the evolution machinery is non-vacuous · refs coherence-audit SK-3/HK-2/HK-3, ADR-70 (Tier-1 Stop-hook propose-closures) · PREMISE-REFUTED 2026-06-05 (annotation only — NOT withdrawn or narrowed; awaiting operator decision): the Self-Evolution memory machinery this item would make non-vacuous (corrections/observations/sessions/violations .jsonl, evolution-log.md, /evolve, /boot, both echo-hooks) was ARCHIVED in Phase C3 — see JOURNAL 2026-06-05 "Phase C3 runtime-machinery cleanup" + docs/audits/2026-06-05-machinery-inventory.md §1/§3/§5. Consumer check 2026-06-05: NO live consumer reads the evolution .jsonl paths or evolution-log.md — ~/.claude/settings.json hooks reference none; /evolve + /boot live only in ~/.claude/archive/2026-06-05-machinery-c3/; ~/.claude/memory/ is empty. So both halves are moot (create-logs feeds retired machinery; repoint-to-evolution-log has nothing live to point at). The "+ one read-only session-close automation" sub-clause was already met by ADR-70 Tier-1 (Stop→propose_closures, live). Operator options: withdraw, or narrow to the session-close sub-clause (pattern: how #87 was withdrawn)
