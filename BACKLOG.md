...
- [#73] [P2][L] Bundle Tier-1 (Stop-hook propose-closures + ruff gate + /review-closures + SessionStart surfacing) as one methodology plugin and install it across all registered ecosystem repos — **5a+5b done**: plugin packaged + installed/verified on all 4 child repos (ai-council, corp-monorepo, corp-ops, corp-sca); remaining → 5c converge `.dev-knowledge` itself onto the plugin (retire the bespoke Unit-1..4 wiring in favour of the installed plugin) · Done when: a single plugin install propagates the Tier-1 lifecycle to every registered repo with no per-repo re-derivation · refs ADR-70 (Tier 1)
