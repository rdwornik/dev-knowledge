...
- [#78] [P3][S] Consolidated docs-refresh pass for the freshness-gated edits deferred to avoid `last_reviewed` re-trips: ARCHITECTURE.md "12 checks" count-hardcode → point at `audit.py checks`, and CLAUDE.md §11 "last 5 ADRs" rotation to include ADR-71 · Done when: both land in one genuine end-to-end re-read with `last_reviewed` re-stamped · refs ADR-71, JOURNAL 2026-06-03 (audit-self-documenting brick)
...
- [#79] ~~[P2][L] Codemap generator layout-coupling — ground each repo's codemap (hand-authored-deliberate vs stale + layout generator-compatibility) and decide fix-generator vs marker-aware-gate, before any codemap rollout. Blocks codemap universalization; TOC rollout is unaffected.~~ **RESOLVED 2026-06-03 — no build needed.** · Done when: grounding complete + decision recorded ✓ — all three codemap repos deliberate hand-authored (`not generator-managed`) + generator-INCOMPATIBLE; marker-aware gate is a no-op on every child (cargo-cult); generator fix unneeded; corp-sca S-scale text-only override. Codemap in correct per-repo end-state. See `docs/audits/2026-06-03-codemap-grounding.md`; end-state note in ADR-71. · refs ADR-71, ADR-51, closes [#79]
