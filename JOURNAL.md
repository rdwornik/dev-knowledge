...
**Changes:** `protocols/ESSENTIALS.md` (Leg 1 table+prose+frontmatter; Leg 2 pointer); `BACKLOG.md` (+#216 under Canonical-file integrity › Keep canonical files accurate, serialize-group: playbook). Commits `89be624`/`c61451a`/`bbd7357` + this wrap.
...
**#77 — NOT closed (conflict surfaced, operator ruling pending).** The operator's standing instruction was to close #77 as "honest size achieved," but at-source #77 is a 4-leg `[P3][L]` protocols-consolidation: only **leg 3** (ESSENTIALS currency) is advanced here; **legs 1/2/4 are PLAYBOOK work** (extract §18; reconcile the 3 divergent lesson→rule statements — the brief's "#77 legs" at PLAYBOOK L2307/L2924, **not** in ESSENTIALS; strip Section-history). A **live disposition** (`warn-77-voided-closure`) asserts "#77 was re-scoped, not done, so it legitimately remains open." Closing now would falsely claim legs 1/2/4 done, orphan the disposition (stale-decoration), and repeat the voided 2026-06-09 `77e5d7d` misattribution. **Recommendation: keep #77 open** (or close + successor-ticket the 3 PLAYBOOK legs + remove the disposition) — operator's call.

**Next:** (1) Operator ruling on #77 disposition. (2) Operator integrates `docs/essentials-batch` (do not FF; `--no-ff`). (3) #216 reconcile PLAYBOOK §2 effort wording.
