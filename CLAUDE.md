...
- **Never restate a count or roster in prose** — cite the surface that computes it (`python scripts/audit.py checks`, `ecosystem/doc-counts.md`, `ecosystem/organ-index.md`, the manifest `carriers:` block). A number typed into a doc is stale at the next commit: ARCHITECTURE Ch2's pre-commit gate list proved it **three times** before the enumeration was retired in favour of its pointer (2026-08-23).
...
> _Entries v1.0–v2.63 condensed to git history per ADR-49/65 (info-preserving — full prior history: `git log --follow -p -- CLAUDE.md`)._
