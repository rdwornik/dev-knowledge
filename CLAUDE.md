...
- **Portable layer:** root `AGENTS.md` carries the build/test/landing facts every provider reads (ADR-115, superseding ADR-53 Decision 2); this file keeps the Claude-runtime remainder and imports it below.
...
> _Entries v1.0–v2.67 condensed to git history per ADR-49/65 (info-preserving — full prior history: `git log --follow -p -- CLAUDE.md`)._

- v2.68 (2026-08-28, batch-1 lane L1 `lane-a-577-agents-md-lockstep`, [#577]+[#584]) — **the hub stops shipping doctrine that contradicts an Accepted ADR, after three consecutive revisions each recorded knowingly leaving it.** **Landed:** §10's *“Narrating or managing AGENTS.md”* anti-pattern — false since **ADR-115** (Accepted 2026-08-25, superseding ADR-53 Decision 2 and amending ADR-101 §1) — is replaced by the anti-pattern that is actually live: copying `CLAUDE.md` wholesale into `AGENTS.md`, which measures **43.50 KiB against Codex's 32 KiB `project_doc_max_bytes` cap and truncates silently**. Landed in **lockstep** with `templates/claude-regions/antipatterns-universal.md` under register licence **Z-G2**, which exists precisely to dissolve the deadlock that stalled this fix: editing §10 alone breaks fleet parity, editing the template alone was outside every prior lane's contract. Region byte-identity **verified mechanically, not eyeballed** — region body 1,075 B == template 1,075 B, and `boundary_headers.py --check` clean. A root **`AGENTS.md`** lands per ADR-115 §3.2's ruled shape: **103 lines** (bound ≤120), **5,270 B**, combined global+root payload **9,161 B = 28.0 % of the 32 KiB cap**; the `~/.codex` precedence collision is resolved **by scope in the file header**, including the third layer `codex/AGENTS.md` that yields `role → doctrine → role` from a cwd at or below `codex/`. §2 gains the portable-layer bullet and the thin `@AGENTS.md` **importer** — never a symlink, which git materialises as plain text under `core.symlinks=false`. **Measured, not asserted:** the `silent_rule_ratchet` delta is **443 → 443, ZERO** — the operator's pre-authorization for this lane went **unused**, because the replacement bullet was authored token-free rather than by requesting headroom against a baseline with none. **Budget** via `validate_doc_rot.scan_file_budget`: opened **197/200**; condensing v2.65–v2.67 into the git pointer per ADR-49/65 freed **6** counted lines (3 bullets + separators) on the v2.65/v2.66 precedent; spent **5** (§2's three lines, this bullet + its separator). Closes **196/200, headroom 4** — bought, not shaved. **Fixed in passing:** v2.67's bullet sat *outside* the `section-history` region marker; v2.68 sits inside it. **NOT taken, and named rather than left to be discovered:** `[#577]`'s done-when also asks for a **test** asserting the combined payload in bytes against the cap — `tests/` is outside this lane's frozen write-scope, so the byte figures above are measured and reported but **unguarded by a gate**; handed to the integrator as a candidate. §6 step 5's bare `pytest --collect-only` remains owed (ADR-106 defect inside a hub region needing a lockstep template act). L10 version 2.67→2.68.
