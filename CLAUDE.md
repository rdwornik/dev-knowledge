...
> _Entries v1.0–v2.55 condensed to git history per ADR-49/65 (info-preserving — full prior history: `git log --follow -p -- CLAUDE.md`)._

- v2.62 (2026-08-16, batch-6 wrap · the second site v2.61 left standing) — **§3's duplicate of the §5 rule-4 claim is closed, so the file states the Layer-2 invariant once and accurately.** v2.61 re-scoped §5 rule 4 and recorded that its own genuine re-read had found §3 line 49 carrying the identical falsity (*"markdown governance files + read-only validators only"*), knowingly leaving it because the ARC's ruling scoped that act to rule 4 — the v2.55 move. The architect scheduled the remainder as a **standalone wrap act with its own full re-read**, which is what this is; §3 now converges on the same wording rule 4 took, naming hub-local validators, generators and gates as in scope and reserving the prohibition for driving a CHILD repo's state. **Fleet-wide drift on this claim is now 0 sites** — the terminal state v2.53 recorded for the branch-prefix enum and v2.56 for the boot-instruction phrase. `last_reviewed` is NOT re-dated because it already reads 2026-08-16 from v2.61's re-read earlier today; unlike the v2.57/v2.60 cases the re-read here **did** happen, so the stamp is earned rather than merely unchanged — the date is identical only because the acts fall on one day. **NO §9 ROSTER ROW IS OWED by batch 6**, checked rather than assumed: the batch added no hook, `[#533]`'s decomposition adds modules under `scripts/audit_checks/` behind an unchanged facade, and `[#532]`'s ARM split changes a detector's internals, both of which the existing `audit-health` row already covers. L10 version 2.61→2.62. Genuine full-file end-to-end re-read from disk this session (all 12 sections; §9's roster was exercised live against this batch's own 13 spine entries — `block-commit-on-main` REFUSED a direct-to-main repair commit and forced it onto a branch, exactly as its row says, and `audit-health` BLOCKED a merge on `journal_spine_anchor` until the integration entry anchored it); §3 is the one defect the re-read had left to close. **Budget: this edit landed the file exactly ON 200/200**, the state v2.59 named as the one "where the next single-line edit re-reds the gate", so the two oldest bullets (v2.54, v2.55) were condensed into the git pointer above on the v2.59 precedent — ADR-49 and ADR-65 §1 both name git as the destination. No content lost: both remain recoverable verbatim via `git log --follow -p -- CLAUDE.md`. Headroom restored to 2.

- v2.61 (2026-08-16, phase-2 Position 0 · W1 Option B) — **§5 rule 4 is re-scoped onto §10's already-accurate wording, so the rule is true of the tree it governs.** It read *"`scripts/` contains read-only validators only"* while ~23 scripts under `scripts/` mutate state and `scripts/audit.py:4707` pushes to `origin` — descriptively false, filed by phase-1 terra against lane P, ruled R3 as carry-not-edit and recorded by v2.60 as *"knowingly LEFT STANDING"*. The phase-2 architect ruled it TODAY as a standalone Position-0 act (§A3), which satisfies both prior readings: NB3-B wanted the fix, NB3-E wanted it off a wave and given its own re-read — this is that act, not a rider. Option B (re-scope) was taken over Option A (amend): it is the minimum true edit and converges rule 4 with §10's anti-pattern line, which already said *"no scripts that drive state in child repos"* — the same converge-two-disagreeing-sites move v2.56 made under I-D8. The invariant is unchanged in substance: what Layer 2 may not do is drive a CHILD repo's state; hub-local validators, generators and gates were always in scope and now say so. **A SECOND SITE IS KNOWINGLY LEFT STANDING, and this re-read is what found it:** §3 line 49 carries the identical claim (*"NOT a code project — markdown governance files + read-only validators only"*), so the briefing's "single-site edit" premise was wrong. It is filed rather than swept in, on the v2.55 precedent — the ruling scoped this act to rule 4, and widening it silently is how a scoped act stops being checkable. Next window's drive-by, not this one's. Neither the old nor the new text adds a `must|shall|never` token (both open *"Layer 2 never executes"*), so `silent_rule_ratchet` is unmoved. Budget measured live before editing — 196 counted lines of 200, this edit costs 2 → 198. L10 version 2.60→2.61. Genuine full-file end-to-end re-read from disk this session (all 12 sections; §9's roster was exercised live against this arc's own commits — `block-commit-on-main`, `audit-health`, `toc-freshness-playbook`, `validate-hermetization`, `backlog-id-on-close` and `backlog-filing-backpressure` each fired — and §3's duplicate claim is the defect the re-read surfaced); `last_reviewed` re-stamped 2026-08-16.

- v2.60 (2026-08-15, phase-1 batch integration) — **§9's pre-commit roster gains `block-commit-on-main`.** Roster addition only; no doctrine moved. The row was owed to the integrator BY DESIGN rather than forgotten: lane O (`[#527]`) landed the gate and flagged the row as *"OWED TO THE INTEGRATOR, not forgotten"* under the batch-4 W5 precedent (v2.57), which keeps a lane's footprint out of this freshness-gated collision file and pays the debt at the merge — the point at which the gate it describes is actually live in the tree. `doc_claims` had been carrying the WARN that names the single missing entry (`precommit_hook_roster@CLAUDE.md`), and it is the WARN that kept the debt visible; adding this row clears it. The row states the gate's honest limit as well as its behaviour, because lane O's own terra finding is precisely that limit: `current_branch()` fails OPEN on a git error while its sibling `merge_in_progress()` raises, so the module answers the same question two ways — documented and deliberate, but an operator decision rather than an unnoticed bug. **`last_reviewed` is NOT re-stamped**: it already reads 2026-08-15 from v2.59's genuine full-file re-read earlier today, and a same-day roster append would otherwise claim a second end-to-end review that did not happen — the v2.57 reasoning applied unchanged. **§5 rule 4 is knowingly LEFT STANDING** though it is descriptively false (*"`scripts/` contains read-only validators only"* — ~23 scripts mutate state and `scripts/audit.py:4707` already pushes to `origin`): the phase-1 architect ruled R3 that the drift is carried as a packet wrap item and that CLAUDE.md is **not** edited for it this arc, so it is filed rather than smoothed over. Budget checked before editing — 193 counted lines of 200, and this edit costs 2. L10 version 2.59→2.60.
...
- v2.58 (2026-08-12, pre-handoff closing arc — the organ index moves and the guard lands) — **§9's two roster rows change: `organ-index-freshness` now names `ecosystem/organ-index.md`, and `validate-hermetization` gains Rule C.** The index relocated from `docs/ORGAN-INDEX.md` by operator ruling A of 2026-08-11 (register `protocols/STANDING_RULINGS.md` K-1): `docs/` is a Tier-2 GENRE tree under ADR-101 §1 and the organ index is none of the five genres — it is generated ecosystem state, the same class as the `ecosystem/organ-registry.yaml` it reads, and it was the only file that ever sat loose at the `docs/` root. **How it got there is the part worth recording, because it is a gate-shape finding rather than an authoring slip:** ADR-101's refusal gate reads the top level and the `docs/<genre>/` level and stops, so a file loose at `docs/` introduces no new top-level entry and no new genre folder and Rule A was silent **by its own literal spec** — `tests/test_validate_hermetization.py` carried a test asserting exactly that silence. The same commit series therefore closes the CLASS, not the instance: **Rule C** refuses an added file whose home directory is outside an allowlist derived from the live taxonomy, with the message *"new path outside allowlisted homes — operator approval required"*. Rule A's reading is left exactly as it was and its test is kept, now paired with a Rule C assertion, because re-interpreting an existing rule to cover a case it was never written for is how a gate stops being checkable. A live-tree test asserts Rule C admits all 2054 currently-tracked paths, so the allowlist cannot drift from the tree it describes without reddening. `silent_rule_ratchet` measured 440 before and 440 after (baseline 441) — the register entry and both roster rows are phrased declaratively, as `STANDING_RULINGS`' own editing note asks. L10 version 2.57→2.58. Genuine full-file end-to-end re-read from disk this session (all 12 sections confirmed accurate; §9's roster was exercised live against this arc's own commits — `audit-health`, `validate-hermetization`, `audit-index-freshness`, `organ-index-freshness`, `ruff`, `backlog-id-on-close` and `backlog-filing-backpressure` each fired, and the two pre-push organs passed on the night-lane push — and the two roster rows above are the defects the re-read had to fix); `last_reviewed` re-stamped 2026-08-12.
- v2.57 (2026-08-11, batch-4 W5 integration — the owed roster row) — **§9's pre-commit roster gains `organ-index-freshness`.** Roster addition only; no doctrine moved. The row was owed to the integrator BY DESIGN rather than forgotten: the batch-4 execution plan §4.3 offered C-narrow and C-wide scopings for `[#132]`, C-narrow was taken, and it explicitly assigns the `CLAUDE.md` §9 row to the integrator so the lane keeps its footprint out of this freshness-gated collision file — the same reasoning that kept W2 out of `ARCHITECTURE.md`. W5's packet named the debt on the way out; this pays it at the merge, which is the point at which the gate it describes is actually live in the tree. The row states what the gate reads and that a stale organ index is worse than none, because the index replaces *operator-as-registry*. `last_reviewed` is NOT re-stamped: it already reads 2026-08-11 from v2.56's genuine full-file re-read earlier today, and this edit adds one roster line rather than changing anything that re-read verified — re-stamping on a same-day roster append would claim a second end-to-end review that did not happen, which is exactly what the cadence rule exists to prevent. L10 version 2.56→2.57.
