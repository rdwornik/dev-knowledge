# Probe manifest — teeth-y forced primary-source read (v5 §5, execution mode)
<!-- scope: meta -->
...
> probes teeth (`RESIDUAL.md` §4). Do not infer them; run the command. (Execution mode: no
> architect orientation-first probe — that is a §13c addition for architect mode only.)
...
| P3 | What is the current **short HEAD sha**, is the working tree clean, and how many commits is `main` **ahead of/behind** `origin/main`? | live git | the summary holds the generation-time sha + sync-state; a new commit or push moves both (at generation `main` is ahead by 2 — re-derive, don't trust this) | `git rev-parse --short HEAD` + `git status -sb` |
...
| P6 | What integer does `ARCHITECTURE.md`'s `**N collected**` claim state, and what does `pytest --collect-only` collect **right now**? (The first dogfood's DRIFT-1 — now reconciled; this re-checks it.) | `ARCHITECTURE.md` `**N collected**` + live pytest | the live count drifts on any test change; neither integer appears in the residual (the headline names the drift, not the numbers) | `python scripts/validate_doc_claims.py` |
