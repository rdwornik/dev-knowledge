...
# Probe manifest — execution mode: orientation first, then teeth (HANDOFF_PROCESS §5 + §13c)
<!-- scope: meta -->
...
>
> **Branch note.** This bundle was generated on branch `docs/handoff-execution`. This line names only *which*
...
This is an **execution** bundle — no supplement is generated (architect-only artifact, §13), so the beat fires **FULL** — *"what off-repo context: intent, priorities, findings not in the repo, changed decisions?"* — never a narrowed "anything changed since?".
...
| P8 | How many files does a **v5 execution bundle** carry, does it include a **per-bundle `README.md`**, is a SUPPLEMENT file present (and per §13, why not), and where does the stable operator boilerplate live instead? | `docs/handoffs/2026-07-23-dev-knowledge-execution/` listing ∩ `HANDOFF_PROCESS.md` §13 | a summary may "remember" a stale file count or an architect-mode bundle shape; the live bundle + spec are the only ground truth | `ls docs/handoffs/2026-07-23-dev-knowledge-execution/` ∩ `HANDOFF_PROCESS.md` §13 "Bundle shape" (the four-file execution shape; the supplement is architect-only) |
...
4. Probes P2–P10 are *designed to move* between generation and check-time — that is the point. **Answer
   ...
   any `[stale]` line). P8 pins the bundle shape (execution: four files — the supplement is architect-only). P10 grooms the
