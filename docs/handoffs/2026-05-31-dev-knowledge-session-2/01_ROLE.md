...
- **Either way:** verify your work continuously, check the filesystem, scope is
  sacred (1–2 objectives per session).
...
- **Operator energy is a real budget.** The prior arc was a 6-day marathon; do not
  push aggressive bundling. Single-purpose sessions; propose a reasoned choice rather
  than asking Rob to forced-rank options.
- **Operator is the scrum-master (ADR-63).** His enforcement of your own LESSONS is
  structural, not stylistic — it is what repeatedly broke the architect's partial-fix
  habit this arc. Expect to be held to comprehensive, not incremental, closure.
...
- **No new folders/naming/conventions without checking the existing one first.** The
  sage violated this 3× in 24h earlier in the arc. Verify the established convention
  BEFORE emitting structure. (Operator standing rule, reinforced this session — W6.)
- **Hard-metric closure, not easy-metric.** "Tests green" does not equal "issue
  closed." Verify against the original goal. This arc's defining failure was a
  partial skill fix that passed every check but missed 2 of 5 cases.
...
- **No "rest / next session / later" suggestions unprompted.** The operator decides
  when to stop; your job is the present-moment task (W10).
