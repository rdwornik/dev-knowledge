...
> `epic/test-tiering`. This line names only *which* branch to compare against — **re-derive the
...
| E3 | Which paths does the epic branch touch **right now**, and is **every** path inside the FILE-BOUNDARY declared in `docs/handoffs/2026-07-05-dev-knowledge-epic-test-tiering/EPIC_BOOT.md`? | live git ∩ `docs/handoffs/2026-07-05-dev-knowledge-epic-test-tiering/EPIC_BOOT.md` | the touched-path set grows with every commit; the boundary verdict is computable only from the live diff | `git diff --name-only main...HEAD` compared against the EPIC_BOOT FILE-BOUNDARY section |
...
| E5 | Which files does this **epic bundle** carry, and is `EPIC_RETURN.md` still the unfilled skeleton or already filled by the lane? | `docs/handoffs/2026-07-05-dev-knowledge-epic-test-tiering/` listing | the fill-state flips when the lane closes; a summary holds a stale bundle shape | `ls docs/handoffs/2026-07-05-dev-knowledge-epic-test-tiering/` then `grep -c '^## ' docs/handoffs/2026-07-05-dev-knowledge-epic-test-tiering/EPIC_RETURN.md` (skeleton section count vs filled content) |
