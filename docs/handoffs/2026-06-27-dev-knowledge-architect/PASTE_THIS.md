...
> **Generated COLD, then FILLED.** This bundle was generated by **CC at a fresh `/clear`ed session start** (no outgoing architect at generation), capturing the window since the 2026-06-25 handoff from JOURNAL + git + live state. **The operator then FILLED `SUPPLEMENT.md`** from the outgoing architect chat — so its ANSWERS are folded into `PASTE_THIS.md` and **carry this window's strategic off-repo "why" (Q1–Q6 + CC-observed addenda A/B)**. Because the supplement carries answers, the §13(d) operator-context beat **narrows** to the lighter *"anything changed since the supplement was written?"* — it does **not** fire FULL (`HANDOFF_PROCESS.md` §13(d): "refined, not duplicated"). The CC-derived repo-side "why" lives in `RESIDUAL.md`; the supplement is the **off-repo** complement the residual structurally cannot carry. Facts are **four-tag** scoped in `RESIDUAL.md`: recent-window facts reconstructed from JOURNAL/git are **`recall`/`inferred`**; everything CC re-derived live this generation is **`witnessed`** — re-derive the load-bearing ones via `PROBES.md`.
...
>
> **Generated COLD, then FILLED** — generated by CC at a fresh `/clear`ed session start (no outgoing
> architect at generation); the operator then **filled `SUPPLEMENT.md`** from the outgoing chat — its
> ANSWERS are folded into the paste. So the §13(d) operator-context beat in P1's gate **narrows** to
> *"anything changed since the supplement was written?"* — it does **not** fire FULL.
>
...
the paste — Q1–Q6 + addenda A/B), so the beat **narrows** to *"anything changed since the supplement
was written?"* — **not** a FULL re-ask of intent/priorities/findings.
...
4. Probes P2–P9 are *expected to move* between generation and check-time — that is the design. **P7 is
   ...
   ANSWERS **FILLED** — generated cold, then filled; no README). **P9** pins the now-durable serialize-group graph
