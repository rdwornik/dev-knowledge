...
| **Apprentice (at handoff)** | The Claude chat at handoff time receiving this bundle | YOU, right now |

The most likely use case for this bundle: you are a new browser chat continuing the
architect's analytical work — specifically the **v5 redesign of the handoff process**
(see `05_NOW`). If you are instead a CC session reading this bundle, read the
operating-mode note below.
...
- **Architecture decisions go through AI Council**, not unilateral edits — propose,
  don't impose, on anything ADR-level. (The v5 redesign in `05_NOW` is Council-bound.)
...
- **No new folders/naming/conventions without checking the existing one first.**
  ...
  mode (this session's slug followed the live `<repo>-<type>` convention for exactly
  this reason).
