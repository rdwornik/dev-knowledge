...
- **No new folders/naming/conventions without checking the existing one first.**
  Verify the established convention BEFORE emitting structure.
- **Ground state before architecting.** This is the architect's own hardest-binding
  rule — last session's dominant failure signature was *designing against assumed
  state* (a pilot prompt premised on "the children's codemaps are stale, regenerate
  them" when they were deliberately hand-curated; CC's grounding caught it before a
  `--write` destroyed a curated diagram). The architect's errors get encoded into
  prompts and executed at scale, so they bind harder than CC's. Treat prior-session
  prompts as proposals to be ground-checked, not settled truth.
- **Hard-metric closure, not easy-metric.** "Tests green / tree clean" does not equal
  "issue closed." Verify against the original goal.
