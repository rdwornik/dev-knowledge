...
> dates — are deliberately **absent from this whole bundle**. That is what gives the probes
> teeth. Do not infer them; run the command.
...
**Gate:** the architect may not proceed to design until it **holds both orienting lines** —
read live by CC, substring-matched. **Then, before design, the operator-context beat fires
(v5 §13d):** the browser asks the operator one targeted question for **off-repo** context
(intent / priorities / findings not in the repo / changed decisions) — the channel the
repo-derived residual cannot carry. Net: *readable-first is a verified property of the
handoff, with zero content copied*, **then** the off-repo steering is injected.
...
| P3 | What is the current **short HEAD sha**, is the working tree clean, and how many commits is `main` **ahead of / behind** `origin/main`? | live git | the summary holds the generation-time sha + sync-state; at generation `main` was **ahead 1, behind 1** of `origin` (a real divergence — see `RESIDUAL.md` §1) and HEAD/sync move on any commit or fetch — **re-derive, don't trust this line** | `git rev-parse --short HEAD` + `git status -sb` |
...
| P6 | What integer does `ARCHITECTURE.md`'s `**N collected**` claim state, and what does `pytest --collect-only` collect **right now**? | `ARCHITECTURE.md` `**N collected**` (line 265) + live pytest | the live count drifts on any test change; neither integer appears in the residual | `python scripts/validate_doc_claims.py` |

## Gate procedure (CC)

1. **P1 first** — the architect cannot begin design until both orienting lines are read live
   and substring-matched. **Then run the operator-context beat (§13d)** — one off-repo ask —
   before design. Then run P2–P6, each against **live state now** (not generation-time).
...
4. Probes P3/P4/P5/P6 are *expected to move* between generation and check-time — that is the
   design. The pass criterion is **"answered from the live source,"** never "matches the value
   the summary remembered."
