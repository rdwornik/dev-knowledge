...
| P3 | What is the current **short HEAD sha**, is the working tree clean, and how many commits is `main` **ahead of / behind** `origin/main`? | live git | the summary holds the generation-time sha + sync-state; **at this generation `main` was *ahead 4* of `origin/main`, not yet pushed** (the operator-gated push — see `RESIDUAL.md` §1); HEAD/sync move on any commit, push, or fetch — **re-derive, don't trust this line** | `git rev-parse --short HEAD` + `git status -sb` |
