# Handoff — 2026-06-12-dev-knowledge-session-3 — operator runbook

> **Read this top to bottom.** It is the only thing you need to boot a fresh browser (Claude.ai)
> session for this handoff. Mode: **architect**.
> What this session is for: a **planning** session on the way-of-working — **finishing the v5
> handoff machinery deferred at the #149 flip** (BACKLOG theme "Finish the v5 handoff machinery").
...
| File | For | When |
|---|---|---|
| `README.md` | **You** | This runbook. Never pasted. |
...
3. **Say "architect mode."** Tell the browser this is an architect-mode session, so it plans and
   decomposes rather than just reacting.
4. **Hand `PROBES.md` and do P1.** Give the browser `PROBES.md`. It starts with **P1**
   (orientation — it reads the two lines that say what this project is and where this work sits).
   This uses **the run loop** (below).
...
**`run <command>`**. You (or CC) run that command, paste the result back, and the browser checks
...
- **No answers ship.** Each probe carries only a question + the command that yields the answer
  (`HANDOFF_PROCESS.md` §5). The answers — orienting lines, check counts, the HEAD sha, drifted
  ids — are kept out of the bundle on purpose, so the only path to an answer is reading live state.
  A summary can't bluff a forced live read; that is the point ("teeth").
...
`HANDOFF_PROCESS.md` §5) covers verification. This is the **third** architect-mode handoff of
2026-06-12 (the `-3` counter-differentiated slug; `-session` and `-session-2` exist), and it is
the first one generated *after* the operator-first README template (`templates/handoff/v5/README.md.tmpl`)
landed — so this README follows that operator-first shape directly.
