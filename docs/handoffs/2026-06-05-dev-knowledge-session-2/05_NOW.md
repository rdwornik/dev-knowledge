...
- **[#1] Fresh-eyes review for every routine handoff** — extend triangulation from the
  promotion gate to every Phase-2 handoff. Live: v4.4 is beta and its first real
  exercise *is* its promotion test (the built-in measurement).
...
- **Current `main` tip = `e90a1fe`** (the v44-wrap handoff bundle + the `2bac2d2`
  template-refresh chore are merged). The sender's narrative referenced `d031d39`, which
  is one session back — see `04_RECENT` facts table.
- This handoff is being generated on its own feature branch (`docs/handoff-2026-06-05-session-2`);
  merge it after Phase 2. No other feature branches; `origin` has only `main`.
- The `docs/machinery-inventory` branch the sender flagged **no longer exists** (verified
  absent at Phase 2 — nothing to adjudicate).
...
- **Do NOT trust any bundle/map prose about repo state without grep** — the
  staleness-map incident is the proof; `[#89]` exists for exactly this. (This very
  bundle was generated from a re-pasted prior interview — its HEAD claim was stale.)
- **Do NOT stamp records with the harness (UTC) date** — local convention governs;
  the harness clock has read one day behind (`06-04` vs the local `06-05`).
