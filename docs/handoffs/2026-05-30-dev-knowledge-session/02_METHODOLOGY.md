...
four tags; spec consolidation at v5 will fold amendments into the body. The
definitions appear in `04_RECENT.md` of this bundle (so you don't need to read the
spec to apply the discipline). This check is now also enforced at lint time by
`audit.py` check #9.
...
forward unknown.** (A PLAYBOOK methodology rule, codified during the v4.2 cycle.)
