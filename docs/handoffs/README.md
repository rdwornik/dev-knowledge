...
---
<!-- scope: meta -->
<!-- CANONICAL OPERATOR RUNBOOK for HANDOFF_PROCESS v6/v7 bundles (v5 bundles too — see the
...
> session for any v5, v6 or v7 handoff. **Start at the bundle's `HANDOFF_BOOT.md`** — it sends you here for
> the process and tells you which file to paste first.

> **To *generate* a handoff** (which mode, what exactly to type): the `/handoff` command
> (`.claude/commands/handoff.md`) and `uv run --locked python scripts/gen_handoff.py --help` — the
> code describes itself (ruling O-5, 2026-09-23: the PLAYBOOK is human documentation, never the
> authority for a command). **This file is the other half** — how to *consume* a generated bundle
> (boot a fresh browser from it).
...
one level up. There is no per-bundle README; this is it. Some bundles carry more files: a `PLAN.md`
(the #301 era's operator-ratified session plan — later bundles, the 2026-09-19 one included, carry
none; the plan now lives on the transport and is recorded under `docs/audits/`), a
`DECISION_LEDGER.md` (a snapshot, below) and `SEAT-BOOT-*.md` seat boots. None of them is pasted.

## Who each file is for

**You paste exactly one file — `PASTE_THIS.md` — into the browser.** It is the assembled boot payload,
and it is **SHED to what a seat needs on turn one** (LANE-5A-9, 2026-09-24): the session header with
ONE forms line, a three-line **ROLE PIN** (the role file is RESIDENT in the browser project's
instructions since v6.3.0, not re-sent), the residual's **drift-flags + OPEN list + a pointer** to the
rest, the probes, and (architect mode, when filled) a **pointer** to the supplement's answers — built
in order by `scripts/assemble_paste.py`. **It may not exceed 20,000 bytes:** above that the assembler
refuses and writes no paste. Everything shed stays in the bundle — CC pulls it on request.

| File | For | Role |
|---|---|---|
| `PASTE_THIS.md` | **The browser** | **The one you paste.** Header + **role PIN** + residual (shed) + probes + supplement pointer, ≤ 20,000 B. Generated by `scripts/assemble_paste.py`; never hand-edited. |
| `docs/handoffs/README.md` (this) | **You** | The runbook. Generic, never pasted. |
| the bundle's `HANDOFF_BOOT.md` | **You** | The bundle's "start here": names slug · purpose · mode, points at this runbook + `PASTE_THIS.md`, and carries the **Operator-facing forms** card. Forms point to code and data: `scripts/dispatch.py launch --help`, the `templates/*-template.md` seat orders, `ecosystem/provider-registry.yaml`. (Bundles cut before 2026-09-24 render the dispatch form from a PLAYBOOK table that no longer describes the launcher — use `--help`.) Never pasted. |
| `RESIDUAL.md` | source | Its drift-flags and OPEN list are assembled into `PASTE_THIS.md`; the rest (shipped map, next-frontier "why", task-state) stays here — ask CC to pull it. |
| `PROBES.md` | source | Assembled into `PASTE_THIS.md`. Your reference — do not paste separately. |
| `SUPPLEMENT.md` *(architect)* | **You (fill it)** | Always generated. Paste its QUESTIONS to the outgoing architect chat; paste the answers back. The next `PASTE_THIS` carries a **pointer** to its ANSWERS; the incoming seat has CC pull them. See *The strategic supplement* below. |
| `DECISION_LEDGER.md` | **You** | A **snapshot** of the decision ledger at the cut. The live ledger is `to-browser/LEDGER-<repo>.md` on the transport; fold the snapshot forward at the next cut. Never pasted. |
...
> bundle's is your reference; the repo's is **installed once** as the browser project's instructions
> (`protocols/OPERATOR-INTERFACE.md`), and each paste carries only a PIN naming its version + sha.
> Paste **only `PASTE_THIS.md`**.
...
1. **Paste the boot payload.** Open a fresh Claude.ai chat and paste the full contents of
   **`PASTE_THIS.md`** (the one file — it carries the role PIN, the shed residual, the probes, and
   the supplement pointer).
   **First time in a given browser project:** install `protocols/HANDOFF_BOOT.md` as that project's
   instructions first — one-time, per `protocols/OPERATOR-INTERFACE.md`. If the seat reports that
   its instructions do not match the PIN's version+sha, **fix that before continuing**: the pin
   exists so a drifted resident role is loud instead of silent.
2. **Wait for the acknowledgment.** The browser replies with one boot line
   (`Booted as the Layer-1 browser under HANDOFF_PROCESS v7. Ready for CC's handoff.`). If it
...
6. **Work the drift-flags.** Direct the browser to the **`RESIDUAL.md`** section (already in the
   ...
   disagree. The OPEN list under them is carried work; the rest of the residual is one CC pull away.
...
> **Turn one is in the single paste.** Because `PASTE_THIS.md` carries every section a seat needs
> to boot, you do not hand files one at a time — you walk the browser through the sequence (orient →
> beat → residual → finish probes), directing it to each section in turn; anything shed is a CC pull. `PROBES.md` is **one** section worked
...
2. **Paste the answers back** into the **ANSWERS** section at the bottom of the file (combine more than
   one chat if needed).
3. **Tell CC `supplement filled`** — CC commits the file on the handoff branch and re-assembles, so
   the next session's `PASTE_THIS.md` points at the answers (it does not inline them — the 20,000 B
   paste gate).
...
The bundle shape changed over time. Current bundles are **v7**; **v6** is the immediately prior
era (v7 keeps the bundle shape and inverts the AUTHORSHIP of the boot — see §17); older eras are
archived, not deleted.

- **v7 (canonical, 2026-09-01 → )** — the v6 shape, with the boot INVERTED: `/boot-session`
  generates OPERATOR ASKS, FUNNEL HEALTH, north-star arcs, the rot/orphan list and a PROPOSED
  NEXT BATCH from live state; exactly one hand-written RESIDUAL survives (`HANDOFF_PROCESS.md`
  §17).
- **v6 (prior era, 2026-07-31 → 2026-09-01)** — the v5 four-file shape plus the `Destination` boot-header
...
- `protocols/HANDOFF_BOOT.md` — the resident browser role file (installed once as the browser
  project's instructions; `PASTE_THIS.md` carries only its PIN — v7.1.0)
- `scripts/assemble_paste.py` — assembles `PASTE_THIS.md` from the bundle sources; refuses above
  20,000 B
- `templates/{dispatcher-order,integrator-order,batch-common-rules,lane-contract}-template.md` —
  the four seats' orders (`HANDOFF_PROCESS.md` §1)
