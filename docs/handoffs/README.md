...
a three-line **ROLE PIN** (the role file is RESIDENT in the browser project's instructions since
v6.3.0, not re-sent), the residual, the probes, and (architect mode) the supplement's **answers**
when it carries any, concatenated in order by `scripts/assemble_paste.py`. The other bundle files are
its **sources** and your **reference** — you do not paste them separately.

| File | For | Role |
|---|---|---|
| `PASTE_THIS.md` | **The browser** | **The one you paste.** Boot payload — **role PIN** + residual + probes + supplement. One paste; the browser receives everything it does not already hold. Generated by `scripts/assemble_paste.py`; never hand-edited. |
| `docs/handoffs/README.md` (this) | **You** | The runbook. Generic, never pasted. |
| the bundle's `HANDOFF_BOOT.md` | **You** | The bundle's "start here": names slug · purpose · mode, points at this runbook + `PASTE_THIS.md`, and carries the resident **Operator-facing forms** card — dispatch · worktree entry · `/lane-integrate` · the teardown four-step (HANDOFF_PROCESS v7.0.0). Never pasted. |
...
1. **Paste the boot payload.** Open a fresh Claude.ai chat and paste the full contents of
   **`PASTE_THIS.md`** (the one file — it carries the role PIN, residual, probes, and supplement).
...
> **Everything is in the single paste.** Because `PASTE_THIS.md` carries every section, you do not
...
- `protocols/HANDOFF_BOOT.md` — the resident browser role file (installed once as the browser
  project's instructions; `PASTE_THIS.md` carries only its PIN — v7.0.0)
