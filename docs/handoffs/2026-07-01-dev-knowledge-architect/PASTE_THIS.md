...
>
> **Generated LIVE, then FILLED by the outgoing architect.** This bundle was generated by CC at the end
> of a **CC-execution-driven** window (deploy build + audit reconciliation + pre-handoff capture); the
> operator then **FILLED `SUPPLEMENT.md`** from the outgoing architect chat — its ANSWERS are folded into
> the paste (arm-first priority, the floor-A hash-guard reservation, the
> `#226→#230→#222/223→#225→#221` chain). So the §13(d) operator-context beat in P1's gate **NARROWS**
> to *"anything changed since the supplement was written?"* — it does **not** fire FULL.
>
...
the paste — arm-first priority, floor-A reservation, the decomposition chain), so the beat **NARROWS**
to *"anything changed since the supplement was written?"* — **not** a FULL re-ask.
...
4. Probes P2–P9 are *expected to move* between generation and check-time — that is the design. **P7 is
   ...
   ANSWERS **FILLED** — generated live, then filled; no README). **P9** pins the now-durable
   serialize-group graph (#156) with **code-edge = just #218** and **`#220` newly in coherence** (the
   window's tell).
