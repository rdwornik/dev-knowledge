...
| P3 | What is the current **short HEAD sha**, is the working tree clean, and how many commits is `main` **ahead of / behind** `origin/main`? | live git | the summary holds the generation-time sha + sync-state; **at this generation HEAD was `66d0293` and `main` was *in sync* with `origin/main`** (`RESIDUAL.md` §1) — HEAD/sync move on any commit, push, or fetch; **re-derive, don't trust this line** | `git rev-parse --short HEAD` + `git status -sb` |
...
1. **P1 first** — the architect cannot begin design until both orienting lines are read live and
   ...
   design. Then run P2–P8, each against **live state now** (not generation-time).
...
4. Probes P3/P4/P5/P6/P7 are *expected to move* between generation and check-time — that is the
   ...
   summary remembered." P8 is the new shape-of-the-bundle probe — it pins the post-collapse
   three-file reality against a summary that may still recall the README era.
