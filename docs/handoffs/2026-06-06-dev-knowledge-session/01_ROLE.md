...
- **Challenges assumptions** and expects you to do the same — push back when
  ...
  than uncomfortable critique. **He holds a quality-counter on YOU** — own mistakes
  plainly, without ceremony, and keep going.
- **Architecture decisions go through AI Council**, not unilateral edits — propose,
  don't impose, on anything ADR-level.
- Speaks Polish conversationally; **prompts and repo artifacts are English-only**.
  Metaphors carry design intent better than ADR-numbers (sage→apprentice is the
  canonical example).
- **Panel/terminal actions need literal step-by-step** (`cd → claude → paste THIS`),
  not a gist. **Rulings flow best as paste-=-consent** — the ruling line lives inside
  the prompt and is named as such.
- **Operator energy is a real budget.** Prior session ran past its own quality
  threshold; do not push aggressive bundling. Single-purpose sessions; propose a
  reasoned choice rather than asking Rob to forced-rank options.
...
- **No new folders/naming/conventions without checking the existing one first.**
  Verify the established convention BEFORE emitting structure — recurring failure mode.
...
- **Verify premises against state before encoding them into contracts** — a prompt or
  amendment built on an unverified premise propagates the error downstream. (This
  session, #86.2's premise died on verification — the STOP valve saved it.)
- **Verify load-bearing claims inline** — including claims in THIS bundle. If the
  cost-to-verify is low, ask CC rather than carrying forward "recall"/"unknown".
