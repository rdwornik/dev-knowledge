...
repo-derived residual cannot carry. Net: *readable-first is a verified property of the handoff,
with zero content copied*, **then** the off-repo steering is injected.
...
| P3 | What is the current **short HEAD sha**, is the working tree clean, and how many commits is `main` **ahead of / behind** `origin/main`? | live git | the summary holds the generation-time sha + sync-state; **at this generation `main` was *in-sync* with `origin`** (the prior handoff's ahead-1/behind-1 had been reconciled — see `RESIDUAL.md` §1), but HEAD/sync move on any commit or fetch — **re-derive, don't trust this line** | `git rev-parse --short HEAD` + `git status -sb` |
...
1. **P1 first** — the architect cannot begin design until both orienting lines are read live
   ...
   before design. Then run P2–P7, each against **live state now** (not generation-time).
...
4. Probes P3/P4/P5/P6/P7 are *expected to move* between generation and check-time — that is the
