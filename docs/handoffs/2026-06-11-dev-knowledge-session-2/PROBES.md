...
> teeth (`RESIDUAL.md` §"Why the probes have teeth"). Do not infer them; run the command.
...
| P1b | Quote, **substring-exact**, the **opening line** of `ARCHITECTURE.md` Ch1 (`## Purpose [CORE]`) — *where this work sits (Layer 2 of the ADR-28 three-layer model)*. | `ARCHITECTURE.md` `## Purpose [CORE]` (Ch1) | the orienting line is in the live file only; a summary holds a gist, not the substring | `sed -n '/^## Purpose \[CORE\]/,+5p' ARCHITECTURE.md` → the quote must be a substring of the live section |

**Gate:** the architect may not proceed to design until it **holds both orienting lines** —
read live by CC, substring-matched. Comprehension follows from having to surface the exact
lines, not from a paraphrase a summary could fake (v5 §13c). Net: *readable-first is a verified
property of the handoff, with zero content copied.*
