...
| D | **The ship-gate is RED on one undispositioned WARN, and it is this arc's.** | filing intake #30 **verbatim** — as contracted, zero body edits — carried its §E line naming `templates/prompt-template.md`, which creates a 20th `undeclared_edges` prose edge (`docs/intake/2026-08-09-func-verification-organ-and-repeatable-execution.md -> prompt-template`). 19 of 20 are dispositioned under the standing #241 class, **three of them the exact same shape** (`warn-undeclared-intake-25-prompt-template`, `-adoption-consolidation-`, `-v6-proposal-`). `main` before this arc: 26 WARNs, 26 dispositioned, **GREEN**. Not dispositioned here — the register is a governance surface and this arc's contract authorised no write to it | add the 20th register entry under #241 (one line, three precedents) · declare `reconciled_with` on the intake (**stale-edge risk** — an intake is not version-coupled to the template) · leave RED until #241 retires the whole class |
