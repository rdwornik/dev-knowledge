...
## 7. What the architect owes a decision on

Batched, per the decision budget — not dripped. Nothing below was decided here.

| # | Decision | Why it cannot be defaulted | Blocks |
|---|---|---|---|
| **Q1** | **`[#171]` leg 1: (a) implement the ADR-80 writer policy, or (b) rule human-committed satisfies "committed" and amend ADR-86 §2 + the two artifact strings.** | Premise **A** found this fork is **open**, not ruled. R3's instruction *"Do not leave (c)"* means declining to choose is itself the disallowed outcome. | **LANE-L4**, which currently carries leg 1 as an *execution* item at effort `medium`. |
| **Q2** | **`docs/adr/` in AMENDMENT A1 — confirm it means `docs/decisions/`.** | The granted path does not exist (`docs/decisions/` is the ADR home, 87 ADRs). A lane taking A1 literally either stalls or trips `validate-hermetization` Rule A. Correcting an operator amendment is not this seat's call. | Any lane that writes an ADR. |
| **Q3** | **Birth budget.** `banked = 0`, so under R2 arithmetic the batch may file **no** new rows. | Several lanes (L1 explicitly) will produce row *specifications*. Whether any may become a task file needs an explicit grant, since the mechanical cap is zero. | L1, and any lane surfacing a carrier row. |
| **Q4** | **Whether dispositioning the 25 WARNs is sequenced before the batch or before the first `/ship`.** | §3.4 proves a RED ship-gate blocks `/ship` but **not** a push. So this is a real scheduling choice, not a forced precondition — and **21 of the 25 have a CLOSED owner row**, so it cannot be worked off through the ledger as it stands. | The integrator, at first arc close. |
| **Q5** | **Intake `#34`** — items 2–5 of its ratification checklist. | Item 1 is now discharged (§6). ACCEPTED needs a `decided-by`; the seal says no lane may take it. Operator-held by construction. | Nothing in this batch; recorded so it is not silently carried a third window. |

**Two items reported, not raised as questions** — mechanical, needing no ruling:

- `automation/fleet-audit` is **2 commits ahead of origin** (tip `e3fecaf5`). Pushing it clears one
  of the 25 undispositioned WARNs. Not done here: outside this session's write-scope.
- The six `doc_rot` row-length dispositions embed a volatile char count in `match`; **3 of 6 have
  already orphaned** and the other three will orphan on their rows' next edit (§3.5). This is the
  dominant cause of the RED verdict and the cheapest structural fix in the packet.

---

## 8. Session ledger — what this session did and did not do

**Did not:** rule anything · file any row · touch `scripts/nopack_sandbox.py` · hand-edit
`BACKLOG.md` · delete anything · create any new folder · dispatch any session · merge to `main`.

**Did:** land one appended amendment · measure the ledger · enumerate the ship-gate ·
verify six premises against quoted sources · model two collision points · resolve `#34`'s blocker.

**Mutations to tracked files, complete list:**

| File | Change |
|---|---|
| `docs/handoffs/2026-08-23-dev-knowledge-architect/HANDOFF_BOOT.md` | +12 lines: a new `## Amendments` heading and AMENDMENT A1, appended |
| `docs/audits/2026-08-23-technical-phase0-preconditions.md` | new file — this packet |
| `docs/audits/README.md` | regenerated by `gen_audit_index.py --write` (gated; not hand-edited) |
| `JOURNAL.md` | one appended entry |

No file under `tasks/`, `scripts/`, `ecosystem/`, `protocols/`, `docs/intake/` or `docs/decisions/`
was modified. `BACKLOG.md` is byte-unchanged and `gen_task_tree --check` was clean before and after.
...
```
python -m pytest -q --tb=short        (git-bash, PYTHONUTF8=1, unpiped exit code)

1 failed, 3567 passed, 4 skipped, 1 xfailed in 934.17s (0:15:34)
PYTEST_EXIT=1

FAILED tests/test_enforcement_coverage.py::test_anchor_gate_probe_distinguishes_installed_from_absent
       AssertionError: candidate: pre-push hook 'block-unanchored-push' runs block...
```

**Reconciliation:** `3567 + 4 + 1 + 1 = 3573`, which is exactly the collected count pinned in
`ecosystem/doc-counts.md` (*"tests: **3573 collected**"*). No test was added, removed or lost.

### The single RED is INHERITED, and that is provable rather than asserted

Three independent grounds:

1. **This branch cannot have caused it.** `git diff --name-only main...HEAD` returns **three files,
   all `.md`** — the packet, the regenerated `docs/audits/README.md`, and the appended
   `HANDOFF_BOOT.md`. Nothing under `scripts/`, `tests/`, `.pre-commit-config.yaml` or `.claude/`
   was touched. A markdown-only branch cannot change a hook-installation assertion.
2. **The window seal, written today, already names it as the live known RED.**
   `docs/audits/2026-08-23-technical-window-seal.md` §4, *State at the seal*:

> ```
> known RED                 tests/... anchor_gate_probe, inherited since 2026-08-22, tmp_path fixture
> ```

3. **The same seal carries it as an open queue item**, §3:

> **f8-harness** | The `anchor_gate_probe` investigation — RED on `main` since 2026-08-22, rooted
> in a `tmp_path` fixture, diagnosed only as far as *"points at the fixture"*, which is not a
> diagnosis | **Unfiled; the RED is live and inherited by every suite run**

**Survivor REDs against `main`, complete list: exactly one** —
`tests/test_enforcement_coverage.py::test_anchor_gate_probe_distinguishes_installed_from_absent`,
inherited since 2026-08-22, unfiled, owned by queue handle **f8-harness**. **Zero REDs are
