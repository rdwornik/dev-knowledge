...
| **(4) Structured-disposition exemption** — [#364]'s own recorded pair: **(a)** linked evidence file (an `incident-evidence`-class audit artifact — the class already exists in the ADR-101 enum, `validate_hermetization.py AUDIT_CLASS_ENUM`) with the row carrying only a pointer; **(b)** per-ticket declared cap exemption in the validator | (a): zero validator change (`validate_doc_rot.py` measures only BACKLOG task lines, `:91-107` — audit artifacts are outside its scope by construction), **but not zero cost**: each accumulating incident batch now creates, reviews, indexes and retains an immutable audit artifact (CLAUDE.md §5 rule 3 — supersede-only), plus the pointer edit in the row (terra P3, adopted). (b): a validator change + a declaration surface + the fleet-twin ripple; per-ticket so it cannot become a blanket escape | (a) buys **unbounded honest capacity** for the evidence-accumulating class and keeps the cap's accretion job intact for everyone else; the price is one indirection plus the recurring per-incident artifact overhead. (b) buys in-row capacity but weakens the gate by exactly one declared hole per grant and needs its own review-date hygiene |
