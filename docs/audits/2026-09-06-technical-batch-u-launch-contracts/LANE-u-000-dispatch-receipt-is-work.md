# LANE lane-u-000-dispatch-receipt-is-work — make Dispatch-Codespace/Cloud report DONE only against a real origin commit, add the lane-boot preflight, and add the harvest verb to Ch8's dispatch table

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W1-2`. Wave 1.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-dispatch-receipt-is-work` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-dispatch-receipt-is-work LANE-u-000-dispatch-receipt-is-work.md -Effort high
```
...
slug `lane-u-000-dispatch-receipt-is-work` -> branch `worktree-lane-u-000-dispatch-receipt-is-work` -> contract `LANE-u-000-dispatch-receipt-is-work.md`
...
**YOU ARE A TWO-REPO LANE, and the second repo is the one that bites.** `dispatch.ps1` and
`DispatchHelpers` live in **`win-tooling`** (`~/.dev-terminals/bin/`), not in the hub. A lane's
worktree follows the cwd it was **dispatched from**, and this lane is dispatched from the hub root —
so your worktree is a HUB worktree. `EnterWorktree` **cannot** cross repos. Provision the
win-tooling side yourself: `git -C <win-tooling-root> worktree add .claude/worktrees/<slug> -b
worktree-<slug>`, then work through absolute paths, and **commit in BOTH repos** — two branches, two
HANDBACK lines. Say so explicitly in your SESSION file so the integrator knows to walk two queues.

**`win-tooling` has one PRE-EXISTING environment RED in its suite** and no worktree-prune mechanism.
Do not chase that RED; record that you saw it and that it predates you.

**You are the gate on W1-7.** `dispatch.ps1` is yours alone this wave and W1-7 waits on your
HANDBACK. Send it as soon as your commits are pushed.

**Ch8's dispatch table is the SOLE literal-command site** — the harvest verb goes in that table and
nowhere else, or `dispatch_drift` will have two rival sites to reconcile.
...
**NIGHT-2 row `W1-2` — verbatim from the frozen batch file:**

> DAY 3.2 | + `-Model`/provider from the contract reaches the dispatch line (015-B) — the offload row is a declaration until this lands

**And the DAY contract section 3.2 it equals, verbatim — this is your operative text:**

### 3.2 `lane-<L>-000-dispatch-receipt-is-work` — wave 1 · local · **repo: win-tooling** (`~/.dev-terminals/bin/dispatch.ps1`, `DispatchHelpers`) + hub `protocols/PLAYBOOK.md` Ch8 dispatch-table rows only
- **Intent (D).** `Dispatch-Codespace` (and Cloud) report DONE **only** when a commit exists on the lane branch on origin (`git ls-remote` witness); DONE-without-commit = FAILED with the reason. Lane boot check in the codespace: `pre-commit` present, `gh auth status` OK, prompts dir resolved — any miss = FAILED before work starts. Add a **harvest verb** to Ch8's dispatch table (copies `git diff --cached` + working tree of a dead lane to `to-cc/RECOVERED-<lane>.patch` via `gh codespace ssh … cat`, the path that worked today) — copied into the table as the SOLE literal site.
- **Closure.** Seeded empty-result run → FAILED; seeded run with a pushed commit → DONE; harvest verb recovers a seeded uncommitted change. **Before → after:** `empty-result runs reported DONE: 2/2 → 0`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-dispatch-receipt-is-work`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-dispatch-receipt-is-work @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-dispatch-receipt-is-work.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-dispatch-receipt-is-work`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-dispatch-receipt-is-work`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-dispatch-receipt-is-work.md` (≤ 5 KB, "now" section on top).
