# LANE lane-u-000-derived-copies-registry — register every derived copy with source, target and render command, and fail a rebind commit at commit time instead of at ship-gate

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W2-U4`. Wave 2.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-derived-copies-registry` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-derived-copies-registry LANE-u-000-derived-copies-registry.md -Effort high
```
...
slug `lane-u-000-derived-copies-registry` -> branch `worktree-lane-u-000-derived-copies-registry` -> contract `LANE-u-000-derived-copies-registry.md`
...
**MOVING THE FAILURE FROM SHIP-GATE TO COMMIT IS THE DELIVERABLE**, not a side effect. A check that
only fires at ship-gate lets a drifted rebind sit on `main` for a whole window; the accepted
candidate (c) is explicitly *"the check FAILS at commit (pre-commit), not at ship-gate"*.

**`ecosystem/` already exists — you add a file to it, you do not create a folder.** NIGHT-2 section 0
forbids new folders anywhere tonight, and this lane does not need one.

**The witnessed instance that motivated the row is the L0 routing region** (yaml @`5be038ff` vs L0
@`16e11c7`). Start from the registered copies you can witness; a registry entry you cannot render is
worse than an absent one.

**A new pre-commit hook is a coupled surface.** Expect the hook roster, the generated rosters and
`doc_code_coverage_drift` to want updating in the SAME commit — a tree-coherence gate will force it.

**Hand-authored content inside a regeneration marker wedges every future author.** If your check
emits into a generated region, emit the whole region.
...
**NIGHT-2 row `W2-U4` — verbatim from the frozen batch file:**

> local · CC + terra · new `ecosystem/derived-copies.yaml` (existing folder) + check | Candidate (c) ACCEPTED at the sitting: registry of derived copies (L0 routing region, doc-counts, intake index, …) with source → target → render command; a rebind commit must re-render or the check FAILS at commit (pre-commit), not at ship-gate. **Before → after:** `derived copies registered: 0 → N; drift caught at: ship-gate → commit`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-derived-copies-registry`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-derived-copies-registry @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-derived-copies-registry.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-derived-copies-registry`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-derived-copies-registry`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-derived-copies-registry.md` (≤ 5 KB, "now" section on top).
