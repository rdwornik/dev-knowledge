# LANE lane-u-000-erratum-aj-second-pass — write the one-page erratum on the AJ second pass headline and set the intake's status to ACCEPT

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W2-F5`. Wave 2.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-erratum-aj-second-pass` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-erratum-aj-second-pass LANE-u-000-erratum-aj-second-pass.md -Effort high
```
...
slug `lane-u-000-erratum-aj-second-pass` -> branch `worktree-lane-u-000-erratum-aj-second-pass` -> contract `LANE-u-000-erratum-aj-second-pass.md`
...
**AN ERRATUM, NEVER AN EDIT.** `docs/audits/` is immutable (critical rule 3). The 6.4x/16.2x
headline stays exactly where it is; you write a NEW dated artifact beside it. F-5 says this in as
many words: *"a dated erratum audit, never an edit of the immutable audit."*

**CITE THE INTAKE BY PATH: `docs/intake/2026-09-05-tech-aj-second-pass.md`.** `intake-id: 70` is
allocated TWICE in the live tree and a bare `#70` resolves into the wrong namespace. **W1-5 renumbers
one of the pair tonight** — read its HANDBACK line before you write your citation, and if W1-5's
result is not yet on `main`, cite by path only and say so.

**The intake is ACCEPTED — say why, and say what is broken in the same breath.** F-5's ruling is
*"its deliverable exists and is consumed by the memo"* AND *"erratum owed on its headline (evidence
not in tree; integrator HIGH)"*. Both halves, or the record is flattering.

**Your deliverable name carries the enum class token:**
`docs/audits/2026-09-06-technical-erratum-aj-second-pass.md`.

**Three things belong in it:** the claim, what it rests on, and what would verify it. The third is
the one that makes it useful — an erratum that only retracts leaves the next reader where you found
them.
...
**NIGHT-2 row `W2-F5` — verbatim from the frozen batch file:**

> local · filings-N2 · `docs/audits/<date>-technical-erratum-aj-second-pass.md`; intake #70 status ACCEPT | One-page erratum: the 6.4×/16.2× headline is unverifiable in-tree; what it rests on; what would verify it.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-erratum-aj-second-pass`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-erratum-aj-second-pass @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-erratum-aj-second-pass.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-erratum-aj-second-pass`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-erratum-aj-second-pass`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-erratum-aj-second-pass.md` (≤ 5 KB, "now" section on top).
