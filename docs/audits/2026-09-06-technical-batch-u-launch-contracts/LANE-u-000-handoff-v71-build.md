# LANE lane-u-000-handoff-v71-build — build the v7.1 handoff pack: bundle under 20 KB, P8's second leg, the P11 decision-carriage probe, and the HANDOFF_BOOT PIN re-issue

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W1-9`. Wave 1.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-handoff-v71-build` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-handoff-v71-build LANE-u-000-handoff-v71-build.md -Effort high
```
...
slug `lane-u-000-handoff-v71-build` -> branch `worktree-lane-u-000-handoff-v71-build` -> contract `LANE-u-000-handoff-v71-build.md`
...
**`protocols/HANDOFF_BOOT.md` HAS 636 BYTES OF HEADROOM AND A HARD GATE.** Measured at step 0:
17,364 bytes against `boot_byte_budget`'s 18,000-byte ceiling. You are re-issuing its PIN and fixing
two names inside it. **Measure before you commit.** If the budget would be exceeded, that is a
finding for your packet and a HOLD on that hunk — not a budget you raise.

**The handoff templates hard-code `v5` in more than one place**, including the `v5` literal in
`PROBES.tmpl` your contract names, and the template directory is itself literally `templates/handoff/v5/`.
Fix the literals your contract names; do not rename the directory (that is a wider act than this
lane owns, and it would move files the deploy manifest carries).

**`gen_handoff` refuses to run while any worktree is live** — and several are, all night. Plan your
closure around `/handoff-verify` on a **seeded** bundle, which is what your contract asks for, rather
than around generating a real one.

**Do NOT generate a handoff for this session or this batch.** NIGHT-2 is explicitly a no-handoff
window. You build and verify the v7.1 machinery; you do not exercise it on tonight's work.

**`to-browser\PIN-2026-09-07.md` carries the new sha256 for the operator's project upload** — that
file is a deliverable, and it goes to the transport, never to Downloads (D-G).
...
**NIGHT-2 row `W1-9` — verbatim from the frozen batch file:**

> #68 ACCEPTED (D3) | files: `templates/handoff/v5/*.tmpl`, `.claude/commands/handoff.md`, `scripts/verify_handoff_probes.py`, `protocols/HANDOFF_BOOT.md` (PIN re-issue), `protocols/HANDOFF_PROCESS.md` (v7.1 delta). **Intent:** bundle ≤ 20 KB; probes prose once; supplement cites files only (P8) + STATUS size check (≤ 5 KB, "now" first) as P8's second leg; **P11 decision-carriage probe** (031 §2: every DECLARE/AMEND/BATCH of the window has `carried-by:` resolving on main or named OPEN in the residual); the `v5` literal in PROBES.tmpl fixed; the two HANDOFF_BOOT.md names fixed; floor lists **six** items (sixth: decisions are files with a carrier); interface section carries the Batch-communication pointer (rule 2 act 5), the model-switch line with cost (DECLARE-BROWSER-TOPOLOGY §1), and the transport grammar table pointer. HANDOFF_BOOT.md is stamped in the same commit as the PIN re-issue — `to-browser/PIN-2026-09-07.md` carries the new sha256 for the operator's project upload. **Closure:** `/handoff-verify` on a seeded bundle: P8 both legs + P11 PASS; bundle size printed. **Before → after:** `bundle 26 KB → ≤ 20 KB; probes P11: absent → present`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-handoff-v71-build`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-handoff-v71-build @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-handoff-v71-build.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-handoff-v71-build`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-handoff-v71-build`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-handoff-v71-build.md` (≤ 5 KB, "now" section on top).
