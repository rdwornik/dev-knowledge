# LANE lane-u-000-intake-id-next-free — compute the next-free intake id across ALL refs, add a duplicate-id check to both intake generators, and renumber the colliding pairs per D8

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W1-5`. Wave 1.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-intake-id-next-free` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-intake-id-next-free LANE-u-000-intake-id-next-free.md -Effort high
```
...
slug `lane-u-000-intake-id-next-free` -> branch `worktree-lane-u-000-intake-id-next-free` -> contract `LANE-u-000-intake-id-next-free.md`
...
**A `docs/intake/` commit needs BOTH generators or the commit is blocked.** `gen_intake_index.py
--write` feeds `docs/intake/README.md` (hooked by `intake-index-freshness`) **and**
`gen_intake_tree.py --write` feeds `docs/intake/manifest.json`, whose omission FAILs `audit-health`
on `intake_tree_coherence`. Run BOTH, every time. Editing a frontmatter KEY REDs the tree manifest;
quoting a frontmatter VALUE breaks the status parse.

**D8 is your ruling and it is narrow:** *"Earlier-merged keeps the id; the later file takes
next-free."* Later = later first commit date, measured, not guessed. **Every citation of a moved id
is updated in the SAME commit.** Ids in play: 14, 42, 70, 72.

**Cite the id-70 pair BY PATH while you work.** `intake-id: 70` is currently allocated twice —
`docs/intake/2026-09-05-tech-aj-second-pass.md` and
`docs/intake/2026-09-05-tech-session-roles-with-a-carrier.md`. A bare `#70` resolves into the wrong
namespace. **W2-F5 cites the aj-second-pass intake by path and depends on your renumbering** — say
in your HANDBACK which file kept 70 and which moved, and to what.

**Next-free must scan ALL refs (local + origin branches), not `main` only** — that is D8's own
mechanism row, and it is the half that makes the collision recur.
...
**NIGHT-2 row `W1-5` — verbatim from the frozen batch file:**

> DAY 3.5 | + duplicate-id check in `gen_intake_index.py` / `gen_intake_tree.py` (filings' Q-3 finding: generators write silently on a colliding tree); renumber the id-70 pair per D8

**And the DAY contract section 3.5 it equals, verbatim — this is your operative text:**

### 3.5 `lane-<L>-000-intake-id-next-free` — wave 1 · local · files: the intake-id allocator (locate: the script/command that assigns `intake-id`), its test; **renumbering per D8**: the later of each pair (14, 42, 70, 72 — later = later first commit date) takes next-free, and every citation of the moved id is updated in the same commit
- **Closure.** Allocator computes next-free across all refs; zero duplicate ids on main; citations resolve. **Before → after:** `duplicate intake-ids: 4 → 0`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-intake-id-next-free`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-intake-id-next-free @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-intake-id-next-free.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-intake-id-next-free`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-intake-id-next-free`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-intake-id-next-free.md` (≤ 5 KB, "now" section on top).
