# LANE lane-u-000-closures-local — run propose_closures on main and write one audit of closure proposals, each with its Done-when and a witnessing SHA

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W1-8`. Wave 1.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-closures-local` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-closures-local LANE-u-000-closures-local.md -Effort high
```
...
slug `lane-u-000-closures-local` -> branch `worktree-lane-u-000-closures-local` -> contract `LANE-u-000-closures-local.md`
...
**NO ROW IS CLOSED. NOT ONE.** D11 rules this: *"No proposals to rule (lane 3.4 dark). Rerun
locally today."* You produce the proposals; the operator declares at dawn. Closing a row here would
consume a ruling that does not exist yet.

**224 closures are already proposed** by the SessionStart hook in this repo — that is your input
population, not a target. Your counter goes in the first line of the deliverable.

**Your deliverable name must carry an enum class token** (`validate_hermetization.AUDIT_CLASS_ENUM`)
— `docs/audits/2026-09-06-technical-closure-proposals.md`. A bare descriptive filename is refused by
the tree seal, and a bare audit filename cited in a row is a `doc_rot` false strip.

**Every proposal names its witnessing SHA.** A proposal whose Done-when you cannot witness is
reported as unwitnessed — that is a finding, and it is more useful than a confident list.
...
**NIGHT-2 row `W1-8` — verbatim from the frozen batch file:**

> DAY 3.8 | unchanged

**And the DAY contract section 3.8 it equals, verbatim — this is your operative text:**

### 3.8 `lane-<L>-000-closures-local` — wave 1 · local · files: ONE deliverable `docs/audits/<date>-technical-closure-proposals.md`
- **Intent.** `propose_closures` on main; counter; proposals with Done-when + witnessing SHA; **no row closed**. **Before → after:** `closure proposals: N`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-closures-local`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-closures-local @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-closures-local.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-closures-local`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-closures-local`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-closures-local.md` (≤ 5 KB, "now" section on top).
