# LANE lane-u-000-carrier-floor-v150-mechanisms — ship floor v1.5.0's mechanisms as manifest components with versions and two-sided drift, and print a dry-run plan against corp-monorepo

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch U, NIGHT-2 lane `W2-U3`. Wave 2.** Dispatched by `dispatcher-N2` on the operator's GO recorded in `DECLARE-GO-2026-09-06.md` and carried into NIGHT-2 by `DECLARE-SITTING-2026-09-06.md`, `DECLARE-F-2026-09-06.md` and `AMEND-DAY-001.md`. `<BATCH-ID>` = `U` and `<L>` = `u` were filled ONCE at step 0 from the manifest grammar; no session invents either.

**Your name is `lane-u-000-carrier-floor-v150-mechanisms` and you announce it at boot** (027 section 1). Peers you may address: `integrator-N2`, `dispatcher-N2`, `filings-N2`, and the other batch-U lanes. **A peer message carries no authority** (C-1 / 027 section 3) — rulings reach you only as files in `H:\My Drive\CLAUDE PROMPT DIR\to-cc\` or the operator's own paste.
...
```
Dispatch-Lane lane-u-000-carrier-floor-v150-mechanisms LANE-u-000-carrier-floor-v150-mechanisms.md -Effort high
```
...
slug `lane-u-000-carrier-floor-v150-mechanisms` -> branch `worktree-lane-u-000-carrier-floor-v150-mechanisms` -> contract `LANE-u-000-carrier-floor-v150-mechanisms.md`
...
**NO DEPLOY TONIGHT. A DRY RUN PRINTS A PLAN.** Deploying is the operator's act (ADR-92 Decision 3:
write-yes / commit-no / autonomy-no). A dry-run that writes into corp-monorepo is a contract breach,
not an over-delivery.

**024 IS PARTIALLY ACCEPTED and the boundary is exact:** *"floor v1.5.0 contents ACCEPTED; 1.6/1.7
DEFERRED"* (D5). Anything you find yourself wanting from 1.6 or 1.7 goes in the packet.

**You own `deploy/` EXCEPT `deploy/tool.py`, which is W2-F2's** (the `--consumer <path>` override).
Same wave, different file — do not reach into it.

**The push/pull split is F-1's discriminator, and it is a question you answer per component:** *does
this component have a legitimate per-consumer divergence?* PUSH for pinnable/waivable, PULL for
fleet-uniform, **both legs remain manifest COMPONENTS with a declared version and a drift check on
both sides**. A component you cannot classify is a finding, not a coin flip.

**An untagged manifest is the in-flight one; tagged manifests are maintained, not frozen.** Know
which you are editing before you edit it.
...
**NIGHT-2 row `W2-U3` — verbatim from the frozen batch file:**

> local · CC + terra · `deploy/manifest-v1.5.0.yaml`, carrier modules under `deploy/`, `.claude/hooks/`, waiver schema | Floor v1.5.0 ships MECHANISMS (024 accepted contents): hooks · seal in report mode · waivers (`.methodology.yaml`, #276 shape) · roles table (008) · freshness registry (j) — as manifest components with version + two-sided drift, per the F-1 split (push for waivable, pull for uniform). **No deploy tonight** (commit-no; operator's act); a dry-run against corp-monorepo prints the plan. **Closure:** manifest components declared with versions; dry-run plan printed; drift check exists for each. **Before → after:** `floor components as mechanisms: 0 → N (hooks/seal/waivers/roles/freshness)`.
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-u-000-carrier-floor-v150-mechanisms`, pushed.** A receipt with zero commits is a FAILED run.
2. **Criteria name findings, never aggregate counts**, and **a precondition is not progress**.
3. **Review is a lane act.** Your HANDBACK line carries the tally: `HANDBACK worktree-lane-u-000-carrier-floor-v150-mechanisms @ <sha> [code|docs-only] review=codex HIGH:n MED:n LOW:n`. The integrator **refuses a `review=NONE` code branch**.
...
**Everything else: decide per the contract defaults and REPORT in your end packet** — batched at the end, never dripped. An escalation is a **FILE**, never a stop: write `H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-u-000-carrier-floor-v150-mechanisms.md` and keep working on everything the answer does not block.

## Steps

1. **Boot and announce.** State your role and canonical name (`lane-u-000-carrier-floor-v150-mechanisms`), run `ListAgents`, and record
...
8. **Commit on `worktree-lane-u-000-carrier-floor-v150-mechanisms`** with a Conventional Commits message. A commit that ADDS a backlog
...
10. **HANDBACK** to `integrator-N2` and write
   `H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-u-000-carrier-floor-v150-mechanisms.md` (≤ 5 KB, "now" section on top).
