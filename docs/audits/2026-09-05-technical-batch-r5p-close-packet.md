...
## ERRATUM 2 — ERRATUM 1 located the `#70` collision in the wrong file (2026-09-06)

*Appended, same reason as ERRATUM 1: this packet is on `main`. Reported by the browser-seat-notes
lane, correcting BOTH the R5P dispatcher's report and my own restatement of it. Verified here
against `main` before appending.*

**The collision is in SOURCE FRONTMATTER, not in the generated index.** ERRATUM 1 said
"`docs/intake/README.md` carries `#70` twice". That is true and it is the wrong locus. The
README's Contents block is **generated** (`scripts/gen_intake_index.py`; hook
`intake-index-freshness`, `<!-- generated by gen_intake_index.py; do not edit by hand -->`), so it
is faithfully rendering two sources that each claim the id:

```
docs/intake/2026-09-05-tech-aj-second-pass.md                 intake-id: 70
docs/intake/2026-09-05-tech-session-roles-with-a-carrier.md   intake-id: 70
```

**This matters because it changes what the fix IS.** Repairing the README would regenerate straight
back, or — worse — would hide the collision behind a clean-looking index while both sources still
claim 70. The fix has to land in one of the two files' frontmatter. A reader of ERRATUM 1 alone
could reasonably have gone to the generated file, which is exactly the failure this packet keeps
recording: acting on a locator without resolving it first.

**Attribution, corrected.** The two colliding docs belong to different lanes — `aj-second-pass` to
the FILINGS-2 session, `session-roles-with-a-carrier` to FILINGS-1. Neither belongs to the
browser-seat-notes lane, which filed `#68` and `#69`, both unique. I had told that seat one of the
`#70`s was theirs; it was not, and the correction came from the seat I had misattributed it to.

**It is a RACE, and that is a distinct class from the other collision live in this corpus.** Two
seats allocated `70` concurrently in the SAME namespace — a next-free-id allocation race. That is
not the cross-namespace ambiguity also present today (a defect-register `E-NN` read against an
intake `#NN`). **They need different fixes:** the race needs an allocator that cannot hand out an id
twice; the cross-namespace case needs qualified citations. Whoever takes **R5P-C2** through intake
should not assume that fixing one addresses the other. R5P-C2 is hereby understood to cover the
RACE; the cross-namespace class is the E-NN/#NN notation work already recorded in the seat-notes
audit.

**One thing that survived by construction, worth copying.** AMENDMENT 9 cites the collided id as
*"intake #70, `docs/intake/2026-09-05-tech-session-roles-with-a-carrier.md`"* — number **and** path.
The path disambiguates, so that citation still resolves correctly. Had it used the bare number, as
house style normally does, it would now point at two files, one unrelated. Qualifying a citation
with its path is cheap enough to be the default, and this is the evidence for it.
