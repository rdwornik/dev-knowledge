...
3. **The gate is ours — with an honest scope split (terra DOC P2):** `gen_task_tree.py --check` (regen-and-diff + full disk reassembly) plus the byte-identity pytest suite RED any drift of the derived tree **relative to the current BACKLOG.md**. They do NOT by themselves detect a consistent rewrite of source+tree together — `--check` derives its expectations from the current source. Source integrity therefore needs its own leg in the contract: viewer runs must leave `git status` clean on BACKLOG.md (an independent committed baseline — the check this spike itself used), and the manifest's `source_sha256` pins which source bytes the committed tree derives from. The schema (filename `<id>-<slug>.md`, frontmatter `id: "[#N]"` byte-exact, foreign keys legal) is fleet-owned; a viewer consumes it as-is or is not the viewer.
...
1. **Identity must be an opaque, byte-exact string field** (`"[#433]"`), never a normalized integer or tool-native id: the first real tool encountered (Backlog.md) has its own id grammar (`task-N`, read-only prefix) and cannot host `[#N]` records at all (witnessed: it ignores them) — adopting its native schema would re-key the space as a consequence, though no in-place re-keying of an existing record was itself witnessed (terra DOC P3 qualifier). The contract's identity column must be declared preserve-verbatim.
...
3. **Typed-row composability holds on real data:** 172 heterogeneous prose rows all fit one row shape (id, status, priority?, size?, theme, story, serialize-group?, depends-on? + verbatim body) with optional fields omitted-when-absent — no parallel store needed. Precisely stated (terra DOC P2): what the pilot establishes is a **frontmatter-typed row over a verbatim body, with a manifest carrying document residue** — not a frontmatter-only representation; the [#382] contract should model all three parts, not just the typed fields.
...
7. **Directory-as-id-counter: substrate built, allocation rule NOT built** (obligation 3 remains open — terra DOC P2 downgrade): the tree gives every id a filesystem representation (one file per id; duplicate ids refused at parse), which is the substrate an allocation rule would be defined over. No next-free calculation exists in the generator, `max` over filenames is not by itself an allocation rule, and no allocation probe was run — obligation 3 is discharged by the restructure ADR defining and witnessing the rule, not by this arc.
...
- **Terra DOC lane** over this report (same invocation form, prompt-mode): **4 P2 + 1 P3, all adopted** — findings and dispositions in §6a.
...
### 6a. Terra DOC-lane findings + dispositions (all five ADOPTED in the completing commit)

1. **[P2] §4 gate overstatement** — "`--check` … derives the expected tree and manifest from the current `BACKLOG.md`, so a viewer that changes `BACKLOG.md` and updates the derived files consistently will pass both `--check` and the byte-identity tests." **FIXED:** §4 item 3 rewritten with the honest scope split — the gate REDs tree-vs-current-source drift only; source integrity is the independent git baseline (clean `git status` on BACKLOG.md) + the manifest `source_sha256` pin, now explicit contract clauses.
2. **[P2] §5.3 "frontmatter alone"** — the row includes the verbatim body and the manifest carries residue. **FIXED:** reworded to frontmatter-typed row + verbatim body + manifest residue; the [#382] contract should model all three.
3. **[P2] §5.7 unproven directory counter** — no allocation rule exists in the generator; `max` is not an allocation rule; nothing witnessed. **FIXED:** downgraded to substrate-built / allocation-rule-not-built; obligation 3 recorded as remaining open to the restructure ADR.
4. **[P2] §6a placeholder vs completion claim** — the committed report asserted a review-lane record while §6a was a placeholder. **FIXED:** this section; the report is complete only as of the commit carrying it.
5. **[P3] §5.1 identity overstatement** — "would silently re-key the space" exceeded the witnessed behavior (the tool ignores `[#N]` records; native grammar is `TASK-N`). **FIXED:** qualified to consequence-of-adopting-its-schema, with the witnessed limit stated.
...
4. **Derived-zone double-reporting (found by the final ship-gate, fixed):** the first final-HEAD ship-gate went RED with 12 new `undeclared_edges` WARNs — every one a `tasks/*.md` byte-copy of a BACKLOG line that names the handoff-process spec (the canonical BACKLOG.md edge is dispositioned under #241). Fixed structurally, not by 12 register rows: `tasks` joined `validate_reconciliation._EXCLUDE_DIRS` (the set whose charter is exactly "a duplicate copy of a dependent cannot double-report"), pinned by a test. Same class as the #335 templates exemption. Feeds [#382] finding 5: a derived surface needs to be invisible to prose-edge organs as a CLASS, or every derived tree re-mints every edge.
5. **Pre-existing suite reds at bare `feb95e18`, witnessed in a throwaway worktree (removed after):** (a) 17 `tests/test_fleet_analytics.py` fails are `ModuleNotFoundError: pandas` — the L5a `analytics` dependency group is outside the default `uv run --locked` sync; with `--group analytics` the same file is 64/64 green (environment class, not code); (b) `test_check_fleet_parity_green_on_live_repo` was already red on the dispositioned ai-council `conftest.py` root-sweep WARN (#430). This arc's `tasks/` entry briefly added a second finding there — cleared by the `root-tasks` parity row (`{hub: MUST, consumer: LOCAL}`, `dir_tracked` probe, the uv-row precedent); the pre-existing ai-council half stays as it was (owned by #430, dispositioned at the gate layer).
