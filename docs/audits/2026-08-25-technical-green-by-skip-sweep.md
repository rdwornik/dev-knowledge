...
## 9. Addendum — family 3: environment-conditional guards

> **AMENDMENT MARKER — appended at INTEGRATION, 2026-08-26, not by Lane X.** This artifact is
> immutable (CLAUDE.md §5 rule 3); this section is an in-file amendment marker, the sanctioned
> form. Authority: operator relay of **sweep family 3** from `win-tooling`, carried in the
> batch-1 INTEGRATE amendment (item 5) — *"verify the sweep artifact covers environment-conditional
> guards (enforcement gated by skipif or tool-presence on the environment it polices); if absent,
> append an addendum with the win-tooling exemplar plus n=2 exemplars derived live from this repo."*
> Verified absent: `skipif` / `tool-presence` / `shutil.which` / `pytest.skip` return **zero** hits
> across §0–§8. **Findings only — nothing is fixed here.** Integration appends evidence; it does not
> re-open a closed lane's build.

### 9.1 Why this is the same defect, one layer out

§0–§8 swept the 46 `ALL_CHECKS` members for a check that **cannot compute its ground truth at run
time** and reports a non-failing status anyway. Family 3 is that defect displaced one layer: the
**proof that an enforcement organ fires** is itself gated on the presence of the very environment
it polices. Nothing reports `unavailable` — the test simply never runs, and the suite prints green.

**Where this sits in the R-F4 taxonomy, stated so the numbering does not drift.** The dispatch
brief (`docs/audits/2026-08-25-technical-dispatch-brief-to-architect.md` §5) asks the R-F4 sweep for
`n=2` exemplars of **three** families: *green-by-skip*, *absence-asserting tests*, and *flag lost
across substrates*. Environment-conditional guards are **none of those three** — they are a fourth,
relayed separately by the operator from `win-tooling` in the batch-1 INTEGRATE amendment, which
names it "family 3". The label is kept as the operator used it; the disjointness from the brief's
list is recorded here so a later reader does not try to reconcile two different "family 3"s.

This is *worse* than an `unavailable` status on the §0 scale, not better. `unavailable` at least
renders as **N/A** in the audit table, so a reader can see that a cell was not measured. A skipped
test renders as a dot in a pytest summary nobody reads line by line, and the enforcement claim it
was written to support survives in prose entirely unqualified.

The rule, in the words of the repo that filed it:

> *"A hygiene test that can be skipped on the machine that breaks hygiene is not a mechanism."*
> — `win-tooling`, `tests/test_repo_root_hygiene.py` docstring

### 9.2 Exemplar 0 — `win-tooling` (operator-relayed; verified live at `5659ecb`, 2026-08-25)

`tests/test_worktree_hygiene.py:60-63` guards **23** tests behind one module-level mark:
...
The exemplar is not that the guard exists — it is **load-bearing and correct** there, since the
scripts genuinely are pwsh-only. The exemplar is that win-tooling **recognised the consequence and
paid to route around it**: `tests/test_repo_root_hygiene.py` exists *as its own file* precisely so
that the root-layout guard inherits no pwsh dependency, and its docstring says so (lines 18-21).
That is the family-3 remedy discovered independently — **when a guard must be environment-gated,
the property that must never be skipped is moved OUT from behind it.**

### 9.3 Exemplar 1 — `tests/test_enforcement_coverage.py` (the Informant Organ polices itself away)

Three fire_tests are gated on the presence of the tool whose enforcement they exist to prove:

- `_HAS_PRECOMMIT = importlib.util.find_spec("pre_commit") is not None` (line 33)
- `@pytest.mark.skipif(not _HAS_PRECOMMIT, reason="pre-commit not installed — commit-time leg unavailable")` at lines **381, 412, 446**

This is the sharpest instance in the repo because of what the module's own docstring claims for
itself: *"the reporter measures whether enforcement FIRES, not whether files are present."* That is
the load-bearing closure contract of the Informant Organ. On any host without `pre_commit`
importable, the commit-time leg of that contract is **never exercised**, the suite is green, and
`enforcement_coverage.py` continues to report `enforcing-local` verdicts derived from candidacy
rather than from firing. The organ built to refuse "files are present" evidence falls back to
exactly that evidence, silently.

### 9.4 Exemplar 2 — `tests/test_floor_conformance.py` (module-level, 21 tests)
...
Same predicate as 9.3, an order of magnitude more blast radius: **21** tests, the whole ADR-93
floor-conformance harness. Its docstring states the proof at stake — *"This proves the armed loop
FUNCTIONS — the guard fails loud on a poisoned floor at both legs, the git hook auto-arms, a real
task flows through the gate — not merely that files are present."* All of it evaporates on a host
without pre-commit, with no surface recording that it did.

### 9.5 Measured status on this host — latent, not active

Measured at integration on the primary checkout:

```
pre_commit spec: True
git: True
```

So **both exemplars execute here** and neither is currently hiding a regression. The finding is
not "these tests are being skipped"; it is that **nothing distinguishes the two worlds.** A host
with pre-commit and a host without produce the same green, and no artifact, ledger or audit cell
records which one ran. Stated plainly because the sweep's own §0 counter-rule cuts both ways:
over-claiming an inactive defect is the other failure mode.

### 9.6 The conforming pattern already exists in-house — this is a fix shape, not a research task

`tests/test_legibility_graph_conformance.py:50-71` solves exactly this, under an explicit operator
contract quoted in its own source:
...
> code<->code status derive from ONE check (operator contract: derive the cell status at runtime
> from the same check the skipif uses — never hardcode 'proven')."*

The remedy generalises to both exemplars above: **propagate the skip predicate into the reporting
surface**, so a skipped proof is rendered as *not proven* rather than absorbed into a green run. A
skipped cell then cannot be tallied as proven — which is the §0 rule ("a check that cannot compute
its ground truth must FAIL, never skip") restated for the proof layer.

### 9.7 Disposition

**CANDIDATE**, routed to the row that already owns the class. `[#583]` scopes itself to *"every
check, validator and gate leg that can report a non-failing 'could not evaluate' status"* — family 3
is that class measured at the proof layer, and the row's own words ("**Scope is the CLASS, not the
instance**") already reach it. **No new row is born for this addendum** — per ADR-111 the finding is
OWNED, and per the batch's filing-backpressure discipline an integration-time finding that an open
row already covers must attach to it, not birth a sibling.

Owner: `[#583]`. Not fixed in this batch, by construction — see the marker above.
