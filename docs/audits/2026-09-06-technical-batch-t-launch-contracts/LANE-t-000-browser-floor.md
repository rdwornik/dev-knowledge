# LANE lane-t-000-browser-floor — the browser-seat floor, rendered

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.13. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-browser-floor` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Lane lane-t-000-browser-floor LANE-t-000-browser-floor.md -Effort high
```
...
slug `lane-t-000-browser-floor` -> branch `worktree-lane-t-000-browser-floor` -> contract `LANE-t-000-browser-floor.md`
...
**THE LOCATION IS NOT IN INTAKE #68 — VERIFIED, so do not go looking and do not invent one.**
`docs/intake/2026-09-05-tech-handoff-process-v71-amendment-pack.md` (`intake-id: 68`) names **no
location** for `BROWSER-SEAT-FLOOR.md`, and neither does its cited source
`docs/audits/2026-09-05-technical-browser-seat-notes.md`. Your own anti-pattern therefore fires.

**DISPATCHER RULING (C-3 contract default + C-8), so you are not blocked:** create **NO repo path**.

- Render the floor and its PIN hash to
  `H:\My Drive\CLAUDE PROMPT DIR\to-browser\BROWSER-SEAT-FLOOR-DRAFT-2026-09-06.md`
  (outside the repo — no gate, no genre admission needed, operator reads it at dawn).
- Build the skills zip to the same `to-browser\` folder.
- Print the PIN hash and confirm it matches the rendered file.
- Do **not** create a folder, and do **not** put the file at the repo root: root holds 8 `.md` files
  and a 9th trips both the fleet-parity root sweep (which lane 3.10 is fixing this same batch) and the
  ADR-101 Tier-1 file class.
- The question is already filed for you as `QUESTION-dispatcher-T.md` item 2 — reference it, do not
  duplicate it.

**Consequence for your HANDBACK:** you may have **no repo commit**. That is expected here and is NOT
the C-6 "receipt with zero commits is a FAILED run" case, because the dispatcher has ruled the write
out of the tree. Say so plainly in your packet and HANDBACK `[ratification-pending]` with the
`to-browser\` paths as your deliverable.

---

## Your contract — section 3.13, VERBATIM from the frozen batch file

### 3.13 `lane-<L>-l-browser-floor` — wave 1 · local · CC + terra (docs-only) · files: `BROWSER-SEAT-FLOOR.md` (location per intake #68), skills zip, PIN hash line · **HANDBACK `[ratification-pending]`**
- **Intent.** Render candidate (l): the browser-seat floor (five probed items: role/loop · where truth is · equilibrium contract · operator rights · session topology) + skills zip + PIN hash — the browser side of the carrier (027 §7).
- **Closure.** File rendered from the v7.1 pack's interface section; zip built; PIN hash printed and matches the file.
- **Anti-patterns.** Inventing a path (the location must come from intake #68 — if it names none, QUESTION file, do not create a folder).
- **MODE** execution. **Pointer:** intake #68 (DRAFT), 022-C. **Before → after:** `browser floor: absent → rendered (held)`.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.13 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-browser-floor`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.13 names — on the operator's surface,
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-browser-floor.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-browser-floor`), run `ListAgents`, and report any
...
4. **Do the work** section 3.13 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-browser-floor`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-browser-floor.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.13 above is binding and is not repeated here** —
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-browser-floor.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-browser-floor`, pushed.
Then send `HANDBACK worktree-lane-t-000-browser-floor @ <sha> [docs-only|code]` to `integrator` and write
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-t-000-browser-floor.md`.

**Print your before -> after line** — the one your section 3.13 names — on the operator's surface.
