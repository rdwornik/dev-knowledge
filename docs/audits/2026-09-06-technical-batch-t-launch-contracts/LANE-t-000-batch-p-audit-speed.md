# LANE lane-t-000-batch-p-audit-speed — batch P — P2/P3/P5, audit-gate speed from measured cost

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.8. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-batch-p-audit-speed` and you announce it at boot** (027 section 1). Peers you may address:
...
**Shape:** `codespace` — the repo's own devcontainer, off-machine, receipt-gated. Commits and
pushes from inside the container, so it pairs a `worktree-` branch, NOT `claude/`.

```
Dispatch-Codespace -Contract LANE-t-000-batch-p-audit-speed.md -Slug lane-t-000-batch-p-audit-speed
```

**CREATE, NEVER REBUILD** (ruling 2026-08-31): a rebuilt container has not applied its own
`devcontainer.json`. If your container arrives without `uv` / `claude` / the provisioning stamp,
**STOP and report** — a lane that repairs its own container destroys the receipt it exists to
produce. Read `Ok` and `RemoteExitCode` separately.

## Worktree pairing

slug `lane-t-000-batch-p-audit-speed` -> branch `worktree-lane-t-000-batch-p-audit-speed` -> contract `LANE-t-000-batch-p-audit-speed.md`
...
**DONE-WHEN, QUOTED VERBATIM (C-6).** Resolved to the **intake** namespace, not `tasks/`: the
contract's phrase "`tasks/` row #71" misnames it. `tasks/71-*.md` exists and is *"reconcile
ENVIRONMENT.md's .claude directory tree"* — a different subject. The real source is
`docs/intake/2026-09-05-tech-batch-p-audit-gate-speed.md` (`intake-id: 71`), section
...
5. P4 only: ruff `N` enabled in `pyproject.toml` + pre-commit, the 13 fixes landed, and ADR-108 SB-4
...
**Criterion 5 is OUT OF YOUR SCOPE** by the contract's own words ("P2/P3/P5 only") — it is carried as
provenance, not as your bar. **Criteria 1-4 and 6 BIND**, and criterion 3 (byte-identical verdicts) is
the one that makes the whole change admissible.

**A LIVE ship-gate baseline you can use, measured at `a39edb2d` in git-bash tonight:** the gate ran to
completion and returned `RED -- 1 hard-fail organ(s); 36 new/undispositioned WARN(s)`. Note that the
tree is RED for content reasons (`routing_agreement`), so your byte-identical-verdict check compares a
RED verdict to a RED verdict — that is still the correct comparison, and it is cleaner than waiting.

**MERGE ORDER — you merge LAST.** You HANDBACK `[ratification-pending]` (#71 is a DRAFT intake) and the
integrator HOLDs you off `main` until the operator's dawn word. Lane `lane-t-000-reds-spine` (3.10)
touches `test_check_fleet_parity` and the spine-anchor check; it is merged and green BEFORE you are
considered. Do not touch tests it owns.

---

## Your contract — section 3.8, VERBATIM from the frozen batch file

### 3.8 `lane-<L>-71-batch-p` — wave 1 · **codespace slot 1** · producer Codex (bounded build), CC verifier, terra · files: per intake #71 P2/P3/P5 only (`scripts/` as the row names them) · **HANDBACK `[ratification-pending]`**
- **Intent.** P2 per-run git index, P3 read cache, P5 verify-in-lane — Done-when VERBATIM from row #71 (dispatcher fill). Measured baseline: git subprocess = 75–79 % of both gates; P1 landed 87 spawns → 1, health −43 %.
- **Closure.** `audit health` wall time before → after on the same substrate, printed; full suite green in the codespace (P5).
- **Anti-patterns.** Rust/C#/GPU (REJECTED, measured); touching checks' semantics; merging (integrator HOLDs until #71 is ratified).
- **MODE** execution. **Pointer:** intake #71 (DRAFT), research 2026-09-05 python-quality-speed. **Before → after:** `audit health: Ns → Ns`. **Library-first:** stdlib caching / a per-run index over a dependency; divergence measured.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.8 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-batch-p-audit-speed`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.8 names — on the operator's surface,
...
the operator's browser channel to-browser/QUESTION-lane-t-000-batch-p-audit-speed.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-batch-p-audit-speed`), run `ListAgents`, and report any
...
4. **Do the work** section 3.8 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-batch-p-audit-speed`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-batch-p-audit-speed.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.8 above is binding and is not repeated here** —
...
the operator's browser channel to-browser/QUESTION-lane-t-000-batch-p-audit-speed.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-batch-p-audit-speed`, pushed.
Then send `HANDBACK worktree-lane-t-000-batch-p-audit-speed @ <sha> [docs-only|code]` to `integrator` and write
the operator's browser channel to-browser/SESSION-lane-t-000-batch-p-audit-speed.md`.

**Print your before -> after line** — the one your section 3.8 names — on the operator's surface.
