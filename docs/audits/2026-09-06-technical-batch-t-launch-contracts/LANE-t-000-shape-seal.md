# LANE lane-t-000-shape-seal — the shape spec DRAFT and the corp-monorepo seal REPORT

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.7. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-shape-seal` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Lane lane-t-000-shape-seal LANE-t-000-shape-seal.md -Effort high
```
...
slug `lane-t-000-shape-seal` -> branch `worktree-lane-t-000-shape-seal` -> contract `LANE-t-000-shape-seal.md`
...
**INTAKE SERIALIZATION — you go SECOND (coupling scan COLLISION 2).** You and lane
`lane-t-000-nc1-clear` (3.6) both add a `docs/intake/` file, and `docs/intake/` is served by TWO
generators that both rewrite shared generated files (`docs/intake/README.md` via
`gen_intake_index.py --write`; `docs/intake/manifest.json` via `gen_intake_tree.py --write`, whose
omission FAILs `audit-health` and blocks the commit). Two lanes writing them concurrently collide.

**Order your work so you are not idle:** do the **corp-monorepo seal REPORT half first** — it is
unblocked and is your read-only deliverable
(`docs/audits/2026-09-06-technical-seal-report-corp-monorepo.md`). **Commit your intake DRAFT only
after `lane-t-000-nc1-clear` HANDBACKs.** Watch for its HANDBACK message; 10-minute timeout falls back
to a file check (`to-browser\STATUS-INTEGRATOR.md`, `git log --first-parent`, or
`git branch -a | grep nc1-clear`). If it has not landed by the time your report is done, commit the
report alone, HANDBACK, and record the intake DRAFT as a deviation in your packet — do not block.

**DO NOT REGENERATE `docs/audits/README.md`** (`[#590]`); the integrator does it once on the merged
result.

**NO WRITE IN corp-monorepo** — the contract says so and C-8 backs it. `validate_hermetization` in
REPORT mode only.

---

## Your contract — section 3.7, VERBATIM from the frozen batch file

### 3.7 `lane-<L>-023-shape-seal` — wave 1 · local (corp-monorepo is another repo; cloud token is hub-only) · CC, adversarial codex on the spec · files: ONE new `docs/intake/` DRAFT + ONE report `docs/audits/<date>-technical-seal-report-corp-monorepo.md`; **no write in corp-monorepo**
- **Intent.** (1) Intake DRAFT "Shape spec + tree-seal shipped to consumers (universalization by mechanism)", theme E6, size L, `carried-by: manifest`, written against inbox 023 ONLY (the operator's definition: identical SHAPE, content each repo's own): root allowlist, folder grammar, required docs, naming grammar, handoff engine present, and the operator's sorting convention (smallest first, LESSONS/ARCHITECTURE at top — filed here, not improvised per folder). Reconcile with the ROOT-R1 census and the ACCEPTED intake "Multi-repo config standardization and fleet conformance at N=8"; cite fleet-readiness §2 ADD/PRUNE tables as the first measured OUT-OF-PATTERN list. 024's packaging rule and staging are cited as an OPEN proposal, not adopted. (2) `validate_hermetization` in REPORT mode against corp-monorepo (read-only) → root list with RELOCATE / RETIRE / WAIVE proposals — the H0 runbook's first step; nothing armed, nothing moved.
- **Closure.** Both files committed; the report's list is the operator's dawn ruling input; the spec names every existing hub mechanism it reuses (ADR-101 seal, dot_prefix_discipline, fleet_parity root sweep) before any new one.
- **Anti-patterns.** Arming the seal; any write in corp-monorepo; adopting 024's staging; a new checker.
- **MODE** execution. **Pointer:** inbox 023/024, ADR-101, intake N=8, fleet-readiness §2/§4. **Before → after:** `corp-monorepo out-of-pattern items: N (RELOCATE n / RETIRE n / WAIVE n)`. **Library-first:** hub mechanisms inventoried first.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.7 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-shape-seal`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.7 names — on the operator's surface,
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-shape-seal.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-shape-seal`), run `ListAgents`, and report any
...
4. **Do the work** section 3.7 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-shape-seal`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-shape-seal.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.7 above is binding and is not repeated here** —
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-shape-seal.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-shape-seal`, pushed.
Then send `HANDBACK worktree-lane-t-000-shape-seal @ <sha> [docs-only|code]` to `integrator` and write
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-t-000-shape-seal.md`.

**Print your before -> after line** — the one your section 3.7 names — on the operator's surface.
