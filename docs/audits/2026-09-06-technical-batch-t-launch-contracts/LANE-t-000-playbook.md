# LANE lane-t-000-playbook — PLAYBOOK + ENVIRONMENT re-read, and Ch8 Batch communication

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.11. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-playbook` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Lane lane-t-000-playbook LANE-t-000-playbook.md -Effort high
```
...
slug `lane-t-000-playbook` -> branch `worktree-lane-t-000-playbook` -> contract `LANE-t-000-playbook.md`
...
**027's message shapes, for the new Ch8 "Batch communication" section, are these (C-9, verbatim):**

```
HANDBACK <branch> @ <sha> [docs-only|code] [ratification-pending]
PACKET-MERGED <batch> @ <sha>
ANCHORS-DRAINED @ <sha>
TEARDOWN-TRIGGER <worktree>
HOLD <branch> <reason>
ESCALATE <item> <reason>
RULING-RELAY <item>            (informational only -- carries NO authority)
```

The seven points to carry are `ARCHITECT-INBOX-2026-09-06-027.md` items 1-7 in
`H:\My Drive\CLAUDE PROMPT DIR\to-cc\`. Read that file; do not work from this summary.

**SPLIT HANDBACK is mandatory here** — freshness fixes `[docs-only]` on their own commit; the 027
section on a SECOND commit tagged `[ratification-pending]` (027 is a DRAFT candidate, #9 on the dawn
list). The integrator merges the first and HOLDs the second.

**Do not rewrite the dispatch table** (anti-pattern). It is the SOLE literal-command site and this
batch's launch lines were copied from it verbatim.

---

## Your contract — section 3.11, VERBATIM from the frozen batch file

### 3.11 `lane-<L>-playbook` — wave 1 · local · CC + terra (docs-only → terra-alone merge) · files: `protocols/PLAYBOOK.md`, `protocols/ENVIRONMENT.md`
- **Intent.** Real re-read of PLAYBOOK (stale since 2026-08-01, +35d): `Last updated` → today with the changes named; new Ch8 section **"Batch communication"** carrying 027's seven points and message shapes verbatim; transport pointer → OPERATOR-INTERFACE §1; FREEZE block (019) pointer. `ENVIRONMENT.md` re-read (+57d) in the same lane (same protocols/ family; coupling-scan may split).
- **Closure.** `canonical_freshness` no longer lists PLAYBOOK/ENVIRONMENT; section exists with 027 shapes; 027 Done-when's remaining legs (`/lane-boot`/`/lane-integrate` printing role+name, the seeded two-session test) are NOT this lane's — listed as follow-ups.
- **Split HANDBACK:** freshness fixes `[docs-only]`; the 027 section on its own commit tagged `[ratification-pending]` (027 is a DRAFT candidate, #9 on the dawn list).
- **Anti-patterns.** Stamp bump without a read; rewriting the dispatch table.
- **MODE** execution. **Pointer:** inbox 027, 019, OPERATOR-INTERFACE §1/§2. **Before → after:** `PLAYBOOK stale +35d → 0; ENVIRONMENT +57d → 0; Batch-communication section: absent → present (held)`.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.11 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-playbook`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.11 names — on the operator's surface,
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-playbook.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-playbook`), run `ListAgents`, and report any
...
4. **Do the work** section 3.11 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-playbook`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-playbook.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.11 above is binding and is not repeated here** —
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-playbook.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-playbook`, pushed.
Then send `HANDBACK worktree-lane-t-000-playbook @ <sha> [docs-only|code]` to `integrator` and write
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-t-000-playbook.md`.

**Print your before -> after line** — the one your section 3.11 names — on the operator's surface.
