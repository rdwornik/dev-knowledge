# LANE lane-t-000-trace-scorecard — the first consumer of the L4 trace store

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.9. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-trace-scorecard` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Codespace -Contract LANE-t-000-trace-scorecard.md -Slug lane-t-000-trace-scorecard
```
...
slug `lane-t-000-trace-scorecard` -> branch `worktree-lane-t-000-trace-scorecard` -> contract `LANE-t-000-trace-scorecard.md`
...
**FOOTPRINT PINNED — coupling scan COLLISION 1, resolved.** Lane 3.1
(`lane-t-000-v2-logs`) runs concurrently and its writer set also named `scripts/fleet_health.py`. The
witnessed write of the flat log files is `scripts/propose_closures.py` (sole writer);
`fleet_health.py` only `glob()`s them. **You OWN `scripts/fleet_health.py` this batch**; lane 3.1
treats it as read-only. If you need to change `propose_closures.py`, that is an ESCALATE, not an edit.

**Check for an existing printer BEFORE adding a module** — the contract says so explicitly. Live on
`main`: `scripts/fleet_health.py`, `scripts/window_metrics.py`, `scripts/trace_writer.py` all exist.
`window_metrics.py` is very likely your printer; grep it before creating anything new.

---

## Your contract — section 3.9, VERBATIM from the frozen batch file

### 3.9 `lane-<L>-trace-scorecard` — wave 1 · **codespace slot 2** · CC + terra · files: `fleet_health` consumer of the L4 trace store, `window_metrics` printer (new module only if no existing printer — check first), their tests
- **Intent.** First consumer of the L4 trace store on `main`: `fleet_health` counts traces/day; scorecard printer prints the ten window numbers so dashboards have numbers.
- **Closure.** `fleet_health` prints traces/day from live store; printer emits the ten numbers; full suite green in the codespace.
- **Anti-patterns.** A dashboard framework; telemetry beyond the two consumers; touching the store's schema.
- **MODE** execution. **Pointer:** H0-PREP L4 close, 026-B line 10. **Before → after:** `traces/day printed: no → yes; scorecard numbers: 0 → 10`. **Library-first:** stdlib json/csv; existing printers grepped first.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.9 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-trace-scorecard`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.9 names — on the operator's surface,
...
the operator's browser channel to-browser/QUESTION-lane-t-000-trace-scorecard.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-trace-scorecard`), run `ListAgents`, and report any
...
4. **Do the work** section 3.9 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-trace-scorecard`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-trace-scorecard.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.9 above is binding and is not repeated here** —
...
the operator's browser channel to-browser/QUESTION-lane-t-000-trace-scorecard.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-trace-scorecard`, pushed.
Then send `HANDBACK worktree-lane-t-000-trace-scorecard @ <sha> [docs-only|code]` to `integrator` and write
the operator's browser channel to-browser/SESSION-lane-t-000-trace-scorecard.md`.

**Print your before -> after line** — the one your section 3.9 names — on the operator's surface.
