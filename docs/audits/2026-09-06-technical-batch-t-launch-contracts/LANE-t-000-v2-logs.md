# LANE lane-t-000-v2-logs — logs/ flat-file retention — fix the CALLER, move nothing

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.1. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-v2-logs` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Lane lane-t-000-v2-logs LANE-t-000-v2-logs.md -Effort high
```
...
slug `lane-t-000-v2-logs` -> branch `worktree-lane-t-000-v2-logs` -> contract `LANE-t-000-v2-logs.md`
...
**FOOTPRINT PINNED — coupling scan COLLISION 1, resolved.** Lane 3.9 (`lane-t-000-trace-scorecard`)
runs concurrently and its footprint also names `scripts/fleet_health.py`. The witnessed write is
`scripts/propose_closures.py:19` — *"ALWAYS writes `logs/PROPOSALS-YYYY-MM-DD.md`"* — the **sole
writer**. `fleet_health.py`, `review_closures.py` and `logs_retention.py` only `glob()` these files.

- **You WRITE:** `scripts/propose_closures.py` (and `scripts/logs_retention.py` only if the caller fix
  genuinely lives there), plus its tests.
- **`scripts/fleet_health.py` is READ-ONLY for you.** Lane 3.9 owns it this batch. If your fix appears
  to need an edit there, that is an ESCALATE, not an edit.

**Witnessed before-count at `a39edb2d`: `logs/` flat files = 133** (`PROPOSALS-*` + `DETECTOR-ERROR-*`
directly under `logs/`). Your before -> after line starts from a re-measured number, not this one.

---

## Your contract — section 3.1, VERBATIM from the frozen batch file

### 3.1 `lane-<L>-v2-logs` — wave 1 · local · CC + terra · files: `logs/`, the writer(s) of `PROPOSALS-*` / `DETECTOR-ERROR-*`
- **Intent.** Witness `ls logs/` flat count NOW. If > 0 dated flat files, the retention grammar hotfix did not hold: find the CALLER still writing flat and fix the caller; move nothing by hand.
- **Closure.** Flat `PROPOSALS-*` / `DETECTOR-ERROR-*` in `logs/` = 0 after one run of every writer; the caller's fix is under test.
- **Anti-patterns.** Moving files instead of fixing the writer; a new detector; touching retention grammar (LANDED).
- **MODE** execution. **Pointer:** retention grammar ruling (batch G #626 residue), 022-B V2. **Before → after:** `logs/ flat: N → 0`. **Library-first:** existing retention module; no new tool.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.1 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-000-v2-logs`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.1 names — on the operator's surface,
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-v2-logs.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-v2-logs`), run `ListAgents`, and report any
...
4. **Do the work** section 3.1 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-000-v2-logs`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-v2-logs.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.1 above is binding and is not repeated here** —
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-000-v2-logs.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-000-v2-logs`, pushed.
Then send `HANDBACK worktree-lane-t-000-v2-logs @ <sha> [docs-only|code]` to `integrator` and write
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-t-000-v2-logs.md`.

**Print your before -> after line** — the one your section 3.1 names — on the operator's surface.
