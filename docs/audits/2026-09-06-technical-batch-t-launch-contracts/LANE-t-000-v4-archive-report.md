# LANE lane-t-000-v4-archive-report — V4 REPORT-STAGE — witness, re-resolve, census; execute nothing

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.3. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-v4-archive-report` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-CloudV2 LANE-t-000-v4-archive-report.md -Title 'lane-t-000-v4-archive-report'
```
...
slug `lane-t-000-v4-archive-report` -> branch `claude/lane-t-000-v4-archive-report` -> contract `LANE-t-000-v4-archive-report.md`
...
`docs/audits/2026-09-06-technical-archive-report-stage.md`. A freehand slug is REFUSED by
`validate-hermetization` Rule B.

---

## Your contract — section 3.3, VERBATIM from the frozen batch file

### 3.3 `lane-<L>-v4-archive-report` — wave 1 · cloud (read-only) · CC · files: ONE deliverable `docs/audits/<date>-technical-archive-report-stage.md`
- **Intent.** REPORT-STAGE (operator's word, DECLARE-GO): (1) witness that the groom lane RAN — `docs/audits/2026-09-05-technical-funnel-groom-sheet.md` on `main`, its pinned SHA, its detectors' outputs; (2) re-resolve EVERY bundle witness at HEAD (locator opened, not cited); (3) 016 folder-rot census per top-level folder → RETIRE-PROPOSED list with witnesses; (4) counts before → projected after per class (intakes, audits, ADRs, tasks). **Executes nothing** — the operator rules the bundles at dawn.
- **Closure.** Deliverable committed + pushed on the lane branch; a copy owed to `to-browser\` is recorded as a residual for a local seat (the groom contract's own resolution), not silently dropped.
- **Anti-patterns.** Any archive/close/retire act; "delete" as a verb; a bundle re-adjudicated (R5 B1–B12 are DECLARED — E-28 review 6a); hand-derived counts a detector computes.
- **MODE** execution. **Pointer:** FUNNEL-GROOM-2026-09-05 contract, 022-B V4, inbox 016. **Before → after:** `archive candidates verified: N/N witnesses resolve; folders with rot: N`. **Library-first:** existing detectors only.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.3 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `claude/lane-t-000-v4-archive-report`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.3 names — on the operator's surface,
...
the operator's browser channel to-browser/QUESTION-lane-t-000-v4-archive-report.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-v4-archive-report`), run `ListAgents`, and report any
...
4. **Do the work** section 3.3 names, inside its stated footprint and no wider.
...
6. **Commit on `claude/lane-t-000-v4-archive-report`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-v4-archive-report.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.3 above is binding and is not repeated here** —
...
the operator's browser channel to-browser/QUESTION-lane-t-000-v4-archive-report.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `claude/lane-t-000-v4-archive-report`, pushed.
Then send `HANDBACK claude/lane-t-000-v4-archive-report @ <sha> [docs-only|code]` to `integrator` and write
the operator's browser channel to-browser/SESSION-lane-t-000-v4-archive-report.md`.

**Print your before -> after line** — the one your section 3.3 names — on the operator's surface.
