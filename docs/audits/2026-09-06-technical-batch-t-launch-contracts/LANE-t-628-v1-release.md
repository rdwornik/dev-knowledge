# LANE lane-t-628-v1-release — the V1 release act — the ESSENTIALS line and the v1.5.0 commit

| Model | Mode | Effort |
|---|---|---|
| opus | execute | high |

**Batch T, contract section 3.14. Wave 2.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-628-v1-release` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-Lane lane-t-628-v1-release LANE-t-628-v1-release.md -Effort high
```
...
slug `lane-t-628-v1-release` -> branch `worktree-lane-t-628-v1-release` -> contract `LANE-t-628-v1-release.md`
...
**LAUNCH CONDITION (already checked by the dispatcher before you were started):**
`PACKET-MERGED wave-1 @ <sha>` from the integrator **AND** ship-gate GREEN on `main` witnessed in
`to-browser\STATUS-INTEGRATOR.md`.

**DONE-WHEN, QUOTED VERBATIM from `tasks/628-dc2-recut-essentials-dissolution-is-a-release-act.md`:**
...
**THIS IS A PARTIAL DISCHARGE AND THAT IS DELIBERATE — read this before you plan.** The row above is
an L-size **dissolution** whose recorded residual is *"the deletion itself, its ten consumers, the
floor sha regeneration and `release_lint` C5"*. Your own anti-patterns **forbid** retiring
`ESSENTIALS.md`, and your files line scopes you to `protocols/ESSENTIALS.md (census only)`. The row
text is therefore carried here as **PROVENANCE**, not as your bar.

**Your operative bar is 3.14's own Closure clause:** region regenerated · the ESSENTIALS first-read
line absent · CLAUDE.md freshness stamp fresh · `ESSENTIALS.md` consumers = N printed as a
RETIRE-PROPOSED count · release commit on the branch, pushed, HANDBACK `[code]`. **`[#628]` stays
OPEN** after you. Filed as `QUESTION-dispatcher-T.md` item 4.

**`git tag` is forbidden** (anti-pattern; the operator declares the tag on the checklist).
**Hand-editing the generated hub region is forbidden** — use its generator; the region body is
byte-matched to `templates/claude-regions/*.md` and a hand edit breaks deploy parity.

---

## Your contract — section 3.14, VERBATIM from the frozen batch file

### 3.14 `lane-<L>-v1-release` — **wave 2** · local · CC + terra · files: `CLAUDE.md` (hub region via its generator), `protocols/ESSENTIALS.md` (census only), release-commit branch
- **Launch condition.** `PACKET-MERGED wave-1` + ship-gate GREEN on `main` in `STATUS-INTEGRATOR.md`. If wave 1 ends RED, this lane is NOT launched; the dispatcher records the blocking organ.
- **Intent.** Remove the CLAUDE.md first-read ESSENTIALS line by regenerating the hub region with its generator (hand edit = defect; fleet-coupled by construction); refresh the CLAUDE.md freshness stamp (the `gated-and-stale` WARN); grep consumers of `protocols/ESSENTIALS.md` → count + RETIRE-PROPOSED (operator's word at dawn); prepare the v1.5.0 release commit on a branch — Done-when VERBATIM from the ESSENTIALS row (dispatcher fill). **The tag is NOT created** — the operator declares it on the checklist.
- **Closure.** Region regenerated; line absent; stamp fresh; ESSENTIALS consumers = N printed; release commit on branch, pushed, HANDBACK `[code]`.
- **Anti-patterns.** `git tag`; hand-editing a generated region; retiring ESSENTIALS.md.
- **MODE** execution. **Pointer:** 022-B V1, #628, tag checklist `docs/audits/2026-09-05-technical-v150-tag-checklist.md`. **Before → after:** `ESSENTIALS line: present → absent; ESSENTIALS.md consumers: N; release commit: none → <sha> (untagged)`.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.14 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `worktree-lane-t-628-v1-release`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.14 names — on the operator's surface,
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-628-v1-release.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-628-v1-release`), run `ListAgents`, and report any
...
4. **Do the work** section 3.14 names, inside its stated footprint and no wider.
...
6. **Commit on `worktree-lane-t-628-v1-release`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-628-v1-release.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.14 above is binding and is not repeated here** —
...
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\QUESTION-lane-t-628-v1-release.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `worktree-lane-t-628-v1-release`, pushed.
Then send `HANDBACK worktree-lane-t-628-v1-release @ <sha> [docs-only|code]` to `integrator` and write
`H:\My Drive\CLAUDE PROMPT DIR\to-browser\SESSION-lane-t-628-v1-release.md`.

**Print your before -> after line** — the one your section 3.14 names — on the operator's surface.
