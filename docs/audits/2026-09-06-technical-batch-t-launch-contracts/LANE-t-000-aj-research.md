# LANE lane-t-000-aj-research — the AJ catalogue and the deployment-model memo

| Model | Mode | Effort |
|---|---|---|
| opus | plan-then-auto | high |

**Batch T, contract section 3.5. Wave 1.** Dispatched by `dispatcher-T` on the operator's GO
...
**Your name is `lane-t-000-aj-research` and you announce it at boot** (027 section 1). Peers you may address:
...
```
Dispatch-CloudV2 LANE-t-000-aj-research.md -Title 'lane-t-000-aj-research'
```
...
slug `lane-t-000-aj-research` -> branch `claude/lane-t-000-aj-research` -> contract `LANE-t-000-aj-research.md`
...
**DO NOT REGENERATE `docs/audits/README.md`** — see the `[#590]` narrowing; three other lanes also
land a `docs/audits/` artifact this batch. The integrator regenerates once on the merged result.

**Your deliverable name carries the `-research-` enum class token:**
`docs/audits/2026-09-06-research-aj-thesis-catalogue-and-deployment-model.md`.

**NAMESPACE WARNING for your intake pointer.** `intake-id: 70` is allocated **TWICE** in the live tree
— `docs/intake/2026-09-05-tech-aj-second-pass.md` (yours) and
`docs/intake/2026-09-05-tech-session-roles-with-a-carrier.md` (027's). Cite yours **by PATH**, never as
a bare `#70`. Filed for the dawn list as `QUESTION-dispatcher-T.md` item 3.

**READ `to-cc\DECLARE-AGY-ROWS.md` BEFORE you brief Gemini** (verified present on disk by the
dispatcher). It carries the ruling your fan-out runs under: the **analysis role is RATIFY REFUSE**;
the **reader role is admitted as measured (6/6) with MANDATORY locator verification by CC**. Your
contract already says Gemini is READER ONLY and never classifies against a doctrine clause — that
DECLARE file is the authority for it.

**TRAP — `ARCHITECT-INBOX-2026-09-05-021.md` section 021-D is STALE on exactly this point.** It still
reads *"agy/Gemini role after #627 FLIP-TO-ADMIT"*. The re-adjudication verdict is **RATIFY REFUSE**
(`docs/audits/2026-09-05-technical-627-readjudication.md`, `## VERDICT`). If anything hands you 021-D
as context, read `DECLARE-AGY-ROWS.md` alongside it and follow the DECLARE.

---

## Your contract — section 3.5, VERBATIM from the frozen batch file

### 3.5 `lane-<L>-022a-research` — wave 1 · cloud (read-only) + Gemini reader · producer CC, fan-out Gemini (retrieval only, every locator re-opened by CC, fabrications counted), adversarial codex on §0 · files: ONE deliverable `docs/audits/<date>-research-aj-thesis-catalogue-and-deployment-model.md`
- **Intent.** File-by-file catalogue of the Architekt Jutra corpus (48 files; reuse `aj-scratch/inventory/INVENTORY.md`) and the master's thesis (inventory first, same P0 shape): claims · mechanisms · tools/CLI/services · artifact shapes · locators; "do we have it / use it / where" answered from live hub reads; vocabulary bridge BEFORE any absence claim. §0 = the deployment-model decision memo (Maister deploy vs our carrier; hub-managed vs independent CC per consumer) measured on fleet-readiness §0, not opinion. Read lane-s's live-comparison result FIRST and cite its 6.4×/16.2× **as unverified** (integrator HIGH: evidence files not in tree). Inputs travel as local paths CC can read — NEVER into any repo; paraphrase + locator only.
- **Closure (022-A, verbatim).** "every catalogue row has a locator on both sides or a recorded empty search; §0 states the model chosen and the first consumer it applies to."
- **Anti-patterns.** A gap table (first-pass failure); Gemini classifying against doctrine; corpus text copied into the repo; the memo as opinion.
- **MODE** plan-mode for §0 only (M-size derivation that can overturn premises); execution for §1–§4. **Pointer:** inbox 022-A, intake #70 (DRAFT — this lane is read-only; its deliverable is an audit, not a build). **Before → after:** `catalogue rows: N, two-sided locators: N, empty searches recorded: N, fabrications: N`. **Library-first:** existing inventory reused.

---

## Done-contract

**Your closure bar is the `**Closure.**` clause of section 3.5 quoted verbatim immediately
...
1. **Done-clause 0 — the deliverable is a commit on `claude/lane-t-000-aj-research`, pushed.** A receipt with zero
...
**Print your `**Before → after**` line** — the one section 3.5 names — on the operator's surface,
...
the operator's browser channel to-browser/QUESTION-lane-t-000-aj-research.md` and keep working on everything the
answer does not block.

## Steps

1. **Boot and announce.** State your role and name (`lane-t-000-aj-research`), run `ListAgents`, and report any
...
4. **Do the work** section 3.5 names, inside its stated footprint and no wider.
...
6. **Commit on `claude/lane-t-000-aj-research`** with a Conventional Commits message. A commit that ADDS a backlog id
...
8. **HANDBACK** to `integrator` and write your `SESSION-lane-t-000-aj-research.md`.

## What NOT to do

**The `**Anti-patterns.**` clause of section 3.5 above is binding and is not repeated here** —
...
the operator's browser channel to-browser/QUESTION-lane-t-000-aj-research.md`.
...
**Commit-and-STOP. Never self-merge** (C-6). Your deliverable is a commit on `claude/lane-t-000-aj-research`, pushed.
Then send `HANDBACK claude/lane-t-000-aj-research @ <sha> [docs-only|code]` to `integrator` and write
the operator's browser channel to-browser/SESSION-lane-t-000-aj-research.md`.

**Print your before -> after line** — the one your section 3.5 names — on the operator's surface.
