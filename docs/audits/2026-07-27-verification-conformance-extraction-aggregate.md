...
| 2026-07-24 | 5 | 3 | 0 | 2 (N1, N2) | 0 | 1 (N3) |
...
- **N2** (Low, L83–L89, still live as-flagged): *"ARCHITECTURE.md (~line 422), CLAUDE.md §9 (~line 172), and CONTRIBUTING.md (~line 114) all list ruff before coherence-nudge in hook order, but `.pre-commit-config.yaml` runs coherence-nudge (line 118) before ruff (line 165)"* → factually confirmed and unchanged on main (ARCHITECTURE.md:455-456). Observation: prose enumeration order is not an explicit execution-order claim; defect-hood is weak.
