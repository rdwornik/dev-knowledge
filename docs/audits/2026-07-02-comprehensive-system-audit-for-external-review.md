...
### 4.8–4.13 The append-only, session, and protocol docs (completing the per-doc coverage)

> §4.1–4.5 covered the living/decision docs; the six below complete the canonical set — the two append-only logs, the two protocol specs, and the two remaining living/gate docs. Each was live-read at this HEAD.

### 4.8 `LESSONS.md` (append-only; Last updated 2026-07-01, `LESSONS.md:6`)
**Purpose:** the append-only institutional record — one dated generalized lesson per entry (`### YYYY-MM-DD | source | lesson | category | [scope: X] | action taken`), newest-at-top in the Entries section, oldest grandfathered (ADR-29). **State:** faithful and current — the recent arc is captured, incl. the full ADR-92 deploy run #1 retrospective + 3 meta-lessons (2026-07-01, `LESSONS.md:12-18`) and the ADR-91 versioning-arc lessons. **Faithfulness:** append-only-**clean** by evidence — `git log -- LESSONS.md` shows a pure append pattern (every recent commit is a `lessons: append/capture …`, e.g. `044c7ef` deploy retrospective, `501d999` versioning arc), and the file's own header restates the invariant (*"New entries go at the top of the Entries section. Never edit old entries. Never delete."* `LESSONS.md:5`). **Gaps:** the append-only invariant is **convention-only, not mechanically guarded** — the `block_immutable_edits.py` PreToolUse guard covers `docs/decisions/transcripts/**` only; **#105 explicitly *removed* handoffs/audits/LESSONS from its scope**, so an in-place edit of an old LESSONS entry would not be blocked fail-closed (it would rely on the git-discipline convention + review). No filed ID proposes a LESSONS immutability guard; the adjacent memory-hygiene item (#130) covers the CC auto-memory + `gotchas` skill, not `LESSONS.md`.

### 4.9 `JOURNAL.md` (append-only, newest-first; the most-gated doc of this set)
**Purpose:** the per-session tactical log (`Did / Result / Changes / Abandoned / Next`), newest-first prepend, the change-record replacing the retired CHANGELOG (`JOURNAL.md:5-18`). **State:** faithful; the recent arc matches the git spine — the 2026-07-02 `#226/#230 CLOSED` entry cites **`b986711`** and the wrap merge **`325db9a`**, both confirmed present on `main`'s first-parent spine (`git cat-file -t` = commit for each; `git log --first-parent` shows 325db9a → b986711 in order). **Faithfulness:** JOURNAL is the **most strongly enforced** doc in this whole set — it is the ADR-85 **hard-block** leg (a session with commits must add an entry naming ≥1 commit SHA from *this session* or the Stop-hook blocks turn-end; §5.3), verified by the un-gameable SHA anchor. So its currency is mechanism-held, not memory-held. **Gaps:** none doc-specific; JOURNAL growth is bounded only by convention (no retention policy), but the flagged unbounded-growth vector is `docs/handoffs/**` (#212), not JOURNAL.

### 4.10 `CONTRIBUTING.md` (last_reviewed 2026-06-27; `reconciled_with: handoff-process@5.3`, `CONTRIBUTING.md:3`)
**Purpose:** branch/commit/validator conventions + the ADR/handoff process pointers for orientation. **State:** current and faithful — it reflects **v5.3 handoff** (*"v5 (stamp v5.3, stable; ADR-82 … amended … 2026-06-25 for the v5.3 §5 probe-manifest consolidation)"* `CONTRIBUTING.md:188`) and the **full current gate set** — its validators table (`CONTRIBUTING.md:97-110`) lists all ten pre-commit hooks incl. `coherence-nudge` and the pre-push `block-ff-push`, matching CLAUDE §9. **Faithfulness:** it is one of the **three** docs carrying the `reconciled_with: handoff-process@5.3` coherence edge (with `ARCHITECTURE.md:3` and `docs/handoffs/README.md:3`); all three declare `@5.3` and HANDOFF_PROCESS's live `Version: 5.3` matches → the `reconciled_versions` gate is green, and it is freshness-gated (in `_FRESHNESS_FILES`), so its 2026-06-27 stamp is honestly enforced. **Gaps:** its validators **table** (`:97-110`) is a hand-maintained mirror of `.pre-commit-config.yaml` that `doc_claims` does **not** reconcile (that check verifies CLAUDE §9's roster + ARCHITECTURE's counts, not CONTRIBUTING's table) — an **ungated drift surface**, currently accurate but not machine-held; no filed ID proposes extending `doc_claims` to it (adjacent to #89's single-doc-accuracy scope).

### 4.11 `protocols/ESSENTIALS.md` (last_reviewed 2026-06-26; freshness-gated)
**Purpose:** the session-start operating frame — the condensed daily driver that **summarizes** PLAYBOOK and points to it (`ESSENTIALS.md:9`). **State:** faithful and current; it carries the ADR-87 architect↔CC contract, the model-is-CC's-pick rule, and the render-layer/output conventions consistent with PLAYBOOK. **Faithfulness:** it is in `_FRESHNESS_FILES` (freshness-gated since the v2.19 doc-currency seal), so its stamp is enforced — but it carries **no `reconciled_with` edge**, so its currency *to PLAYBOOK* is **convention-only** (the "ESSENTIALS summarizes PLAYBOOK, not copies it — divergence is the failure mode" rule; CLAUDE §5 rule 6), not machine-checked. That is the correct-by-design posture (a `reconciled_with` edge to PLAYBOOK would be a version-pin on a non-versioned doc), but it means the summarize↔source drift is caught only by review, not a gate. **Gaps:** the "under-1-page" contract and high-leverage-cheats currency are the open items — **#5** (add ADR-35→76 cheats under the one-page constraint) and the #77 leg-3 ESSENTIALS-trim (voided/re-scoped, legitimately open).

### 4.12 `protocols/HANDOFF_PROCESS.md` (Version 5.3, stable; `HANDOFF_PROCESS.md:4-5`)
**Purpose:** the single canonical source of truth for handoff mechanics (v5 model C — CC-owned residual + thin browser boot + teeth-y forced read; ADR-82). **State:** faithful and current — `Version: 5.3 / Status: stable / Effective 2026-06-11`, supersedes v4.4 (archived), authority statement explicit (`:10`). **Faithfulness:** this doc is the **spec authority** in the coherence spine (`_SPEC_REGISTRY: handoff-process → protocols/HANDOFF_PROCESS.md`); its `Version:` field (5.3) is read **live** and is what the three dependents reconcile against — so drift *between spec and dependents* is gated (FAIL on mismatch). **Gaps:** two. (1) It is the **spec, not a freshness-gated dependent** — it carries no `last_reviewed` stamp and is not in `_FRESHNESS_FILES`, so a **content change without a `Version:` bump would leave dependents silently stale** (the MODIFY / semantic-drift edge the gate cannot see — **#220**, and §7.4). (2) The v5 machinery it specifies is not fully built: the **`/handoff` v5 generator is still #164** (the residual/probe/paste bundle generation), and the stable-probe-core split is #161.

### 4.13 `protocols/DEFINITION_OF_DONE.md` (last_reviewed 2026-06-19; carries `rule: seal-journal-anchor`, `DEFINITION_OF_DONE.md:24`)
**Purpose:** the single source of truth for what "done" means at **session close** (ADR-85) — the JOURNAL hard-block + BACKLOG advisory legs, deterministically enforced by the Stop-hook (no LLM in the gate). **State:** faithful and current — it reflects the 2026-06-19 C1 amendment (*"'This session' = the session boundary, not the push boundary"*, `DEFINITION_OF_DONE.md:30`), and its stamp (2026-06-19) matches that last substantive edit. It correctly scopes itself to the *session-close record only*, pointing build/arc acceptance to PLAYBOOK Ch12.1 + ADR-81 (the three-distinct-scopes discipline, `DEFINITION_OF_DONE.md:89-97`). **Faithfulness:** it carries the `seal-journal-anchor` doc→code edge (so it is in the `doc_code_edge` coverage), and its *teeth* run live (the Stop-hook). **Gaps:** it is **neither freshness-gated (not in `_FRESHNESS_FILES`) nor a coherence-spine dependent (no `reconciled_with`)** — so its own currency is convention-only, backstopped only by the doc→code edge on its one rule. The **BACKLOG leg is still advisory** (promotion to a hard gate is **#168**, blocked on the traceability-spine ADR **#170**), and the 4-week scope-freeze (no new docs added to the gate until ~2026-07-14, `DEFINITION_OF_DONE.md:84-87`) is still in force.
