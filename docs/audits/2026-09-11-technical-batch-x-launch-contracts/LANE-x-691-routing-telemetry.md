# LANE lane-x-691-routing-telemetry — provider routing becomes a measured, admission-gated router - roles, exclusions and a per-repo allowlist, re-ranked by pass rate per cost
...
```
Dispatch-Lane lane-x-691-routing-telemetry LANE-x-691-routing-telemetry.md -Effort high
```
...
`[.dev-knowledge · #691 · lane-x-691-routing-telemetry]`. Model defaults to `opus` — the `.dev-knowledge`
...
outright when `worktree-lane-x-691-routing-telemetry` already exists, so re-running the line is a no-op
...
```
Dispatch-Lane lane-x-691-routing-telemetry LANE-x-691-routing-telemetry.md -Effort high -Model opus
```
...
slug `lane-x-691-routing-telemetry` -> branch `worktree-lane-x-691-routing-telemetry` -> contract `LANE-x-691-routing-telemetry.md`
...
## PRECONDITION — AX22-4, and it gates the non-Claude half of this lane

**No non-Claude producer or reader is ordered before W-2′ (the fail-closed prompts guard) is
on `main`.** At this contract's freeze W-2′ was RUNNING, not merged. So:

- The registry, the exclusions, the allowlist, the router and the telemetry are built and
  tested **regardless** — they are Claude-side work and are not gated.
- **Ordering a non-Claude provider for real work is gated**: until `[#684]` is on `main`, no
  Grok, Copilot Enterprise or agy call is placed as a producer or reader. The bounded trial
  tasks of AX22-1 are part of that gate, not an exception to it.
- If the lane reaches that point and W-2′ is still unmerged, it **reports and stops there**
  rather than deciding the precondition is satisfied enough. Check it, do not assume it:
  `git branch --merged main | grep lane-w-684`.
...
## Two FIRST ACTS, before any of the above

1. **Read the grok account's usage-limit cause** (AX21-4). If it is a spending cap, that is
   the operator's act — one line in the lane's report, not a workaround.
2. **Verify grok's token-ceiling FLAG** (AX21-3, applying R1-4(b)/A7-5). *A CLI that cannot
   enforce a token ceiling by flag is NOT ordered* — so this is a gate on admission, not a
   note. Verify it on this version rather than trusting the docs; `grok -p` is single-turn
   and multi-turn needs `--max-turns`, which is itself part of what must be checked.
...
1. Step 0 sync (below). Then the TWO FIRST ACTS above — the grok usage-limit cause and the token-ceiling flag verification. A CLI that cannot enforce a ceiling by flag is not ordered, so this result gates everything downstream. **COMMIT**
...
5. Re-rank by measured pass rate per cost, over the telemetry from step 2. Report the admission state of each provider. **COMMIT, then STOP.**
