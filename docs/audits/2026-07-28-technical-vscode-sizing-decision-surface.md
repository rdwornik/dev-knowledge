...
| **(c) Re-date-with-reason** | Nothing builds; e1 `review_date`s + the W1 deadline move out with a recorded reason | **S (cross-repo, no build)** — the two `review_date`s live in `corp-monorepo/.methodology.yaml:100` and `ai-council/.methodology.yaml:94`, so this is TWO consumer writes under RULING-W worktree + consumer-leg merge delegation, plus the hub-side W1 deadline edit; coordination cost, not typing cost (terra P2, adopted) | The operator's most-repeated ask stays unmet in corp; W1 stays open past its own deadline; honest only if priorities genuinely changed |
