...
# Governance — .dev-knowledge
<!-- scope: meta -->

> **HOME NOTE — this file is ACTIVE doctrine, not retired.** It lives under
> `docs/archive/` because operator ruling 2026-09-05 requires one new file in an
> EXISTING `docs/` subfolder, and `validate_hermetization.py` Rule C refuses a bare
> `docs/` home. Of the admitted docs subfolders, only `docs/archive/` has no date or
> `ADR-NN` name grammar, and it already holds relocated root doctrine (`VISION.md`).
> The folder's name understates its role; `status: active` in the frontmatter is
> authoritative over the folder name.

> **Relocated from the root `README.md` by lane `lane-h0-readme`** (`[#634]` Batch-G
> filing (a), 2026-09-05): *"README as a human front door. What-it-is, then quickstart,
> then a link map; governance prose relocated and consumers re-pointed."* `README.md` is
> the human front door — what this repo is, why it exists, how to start, where to go
> deeper — and the same genre boundary **R-README** (2026-09-01) already drew for the
> fleet-shape paragraph (moved to `ecosystem/registry.md`) applies here to the rest of
> the governance narrative. Every section below is relocated **byte-identical**; nothing
> is deleted. `README.md` keeps the `## Vision` heading and its opening sentence (P1a) —
> the remainder of that section's body moved here too.

## Front-door provenance
