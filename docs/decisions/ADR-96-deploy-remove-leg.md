...
## Amendment — 2026-07-06 (greenfield remove-leg scope; the P6 re-evaluation this ADR mandated)

**Trigger (n=2, corp-monorepo).** The fleet-scope caveat above fired exactly as written: corp-monorepo carries its **own** consumer-owned ruff (`astral-sh/ruff-pre-commit` @ v0.15.8, own args — adopted independently, never methodology-deployed), which URL-matches the `ruff-gate` tombstone's `prune.match.repo`. `detect_prune` therefore reads `PRESENT_MODIFIED` and the sweep REFUSES — aborting the **entire** first-ever deploy to a consumer the methodology never touched. That REFUSE is a category error, not a conflict: the hash-guard's oracle is the **last-deployed shape** (Decision 2), and with `deployed_methodology_version: null` in the registry there IS no last-deployed shape — a URL-matched artifact on a greenfield consumer is consumer property, not methodology residue.

**Decision (operator-approved at the Arc-4 plan review, 2026-07-06).** `deploy/tool.py` skips the prune sweep entirely when the target consumer's registry record is **null** (never deployed): no `detect_prune`-driven destroy-confirm, no prune, no abort — rendered explicitly in both assess and execute output ("Remove leg SKIPPED — greenfield consumer"). Assess still runs `detect_prune` and prints the table (read-only honesty about what exists on disk). This is **strictly more protective** than REFUSE — the artifact is never even a deletion candidate — and REFUSE semantics are fully preserved for previously-deployed consumers (regression-pinned: `tests/test_deploy_prune.py::test_deployed_consumer_refuse_still_aborts`). The manifest and the v1.2.0 tag are untouched; the tool is engine, not deployed content (the [NB-1] INERT-on-tagged-manifest precedent), so the methodology version is HELD.

**Residual (filed, not built — the no-big-bang boundary again).** After this deploy the consumer's record is non-null, so any future remove-leg run (v1.3.x+) re-enters the sweep and REFUSEs on the same consumer-owned artifact. The durable fix is the D2-class **per-consumer divergence-allowlist the prune leg actually reads** (the P4 `.methodology.yaml` allowlist is Informant-side only), plus FU-1 (#245) last-deployed-bytes tracking — filed as **[#276]**.
