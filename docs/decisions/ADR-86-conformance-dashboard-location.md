...
this repo commits its own output.** The shape all eight generators do follow is *write → print → a
human commits → a `--check` regen-and-diff hook refuses a stale copy* — the shape `BACKLOG.md` has
run on since the ADR-107 §7.2 source-of-truth flip. Option (a) would have made this generator the
first of its kind to self-commit, adding a failure surface (locked index, mid-merge, diverged
`main`) to solve a problem convention had already solved.

**The one place ADR-80 §3 IS implemented, and why it argues for (b) rather than against it.** An
earlier draft of this paragraph claimed the writer policy had *zero* implementations in this repo.
**That was false**, and it is corrected here rather than left standing: `audit.py::_commit_routine_outputs`
(`:3991`) is a full ADR-80 §3 writer — it commits its own durable audit outputs, pathspec-bounded
(Rider 1), fail-soft (Rider 2). The first census missed it because it uses `commit-tree` plumbing
against a separate `GIT_INDEX_FILE`, not `git add` / `git commit`, so a string sweep for those two
verbs could not see it. Found by the terra reviewer, 2026-08-23. **It sharpens the argument:** that
writer exists for an *unattended nightly routine*, and it writes to the orphan
`automation/fleet-audit` branch precisely so `main`'s HEAD, index and working tree are never
touched — the ADR-84 isolation pattern, applied to the case ADR-84 is for. The dashboard is not
that case: it is regenerated by a person, on `main`, as a navigable surface. So the repo's single
ADR-80 §3 implementation is evidence about *which* jobs need a self-committing writer, and the
dashboard is not one of them.
