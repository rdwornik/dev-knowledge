...
- **Source:** Operator ruling, relayed from corp-monorepo 2026-07-19 (binding); recorded here on the session date 2026-07-18 (the corp session date is the provenance label, quoted, not the record date). Refines this ADR's own refusal-gate invariant (§1 tree-seal + §3 gate) — recorded here as an **in-file amendment** per **ADR-94** (in-file amendment = own-invariant refinement, not a new domain), **ADR-102 §1** (record a refinement by *extending the authoritative source* — here, amending this ADR rather than spawning a new one; the standalone registry proposed below **externalizes this ADR's own frozensets into a single source of truth**, which is distinct from the *duplicate parity register* ADR-102 §1 rejected — ADR-102's row also notes it stayed *out* of ADR-101 because it governs parity-schema, not tree/naming), and **ADR-34** (the naming parent this ADR amends).
- **Decision tier:** Path A — an operator-settled refinement of an existing invariant; no Council convene.

**Universal rule (fleet-wide).** *Status: the rule is **adopted** (binding operator ruling); its **mechanical** enforcement — the registry-driven gate (#345) and the executor-behavior agent rule (#346) — is **pending**, so until those land this is **doctrine a reviewer applies and an executor follows, not yet an automatic gate**.* The rule: the new-path gate **must** block convention-**VIOLATING** creations and **never** convention-**COMPLIANT** ones — *convention-compliance IS the authorization.* Two tiers:
