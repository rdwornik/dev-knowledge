...
2. **Two-leg hash-guard, one canonical script.** Both legs run the single `.claude/check_floor_hash.py` (single-sourced from `generate_floor.CHECK_FLOOR_HASH_SCRIPT` — the paste-ready `INSTALL_NOTE` body — so the automated arm and the manual runbook can never disagree on the guard bytes). The legs:
   - **Session-start** — a `SessionStart` command hook in the committed `.claude/settings.json`, run with **`--require-present`**. It travels with the clone (settings.json is tracked), fires every session, is un-suppressible by `git commit --no-verify`, and — because of `--require-present` — fails **loud** not only on a drifted floor but on a **deleted-but-tracked** floor (a silently-absent floor is a #95-class silent drift). A plugin `hooks.json` SessionStart hook was rejected — it registers too late for the one-shot init event and never fires (verified 2026-06-02).
   - **Commit-time** — a `floor-hash-verify` local hook in `.pre-commit-config.yaml` (run permissively) that blocks a commit staging a drifted floor. It **cannot** catch a *pure deletion*: pre-commit passes no files to a `files`-filtered hook when the only change is a deletion, so the hook is skipped. The session-start `--require-present` leg is the backstop for the delete-case (same backstop structure as `--no-verify`). Do not try to force commit-time deletion detection — that fights pre-commit's file-passing model.
...
5. **#230 is the acceptance metric, not files-present.** A conformance harness (`deploy/floor_conformance.py`) proves the loop *functions* end-to-end in an isolated fresh clone (own `.git`, plain-delete teardown — the operator-approved amendment of the acceptance-contract's "worktree file-disjointness"): clean floor passes both legs, a poisoned floor fails loud at both legs, the git hook auto-arms, and a real task flows branch → commit → gate → merge. #226 is DONE only when Layer-2 passes against the **real** ai-council (incl. the tamper leg) — a green Layer-1 synthetic fixture is CI regression, not closure.
   ...
   **Commit-time leg faithfulness (Layer-2 scope — operator-ratified 2026-07-01).** pre-commit initializes *every* repo in a consumer's `.pre-commit-config.yaml` before running any hook, and ai-council's config also carries a relative-path hub-hooks repo (`../.dev-knowledge`, unresolvable from a temp clone) + a remote `ruff` repo (needs network) — both orthogonal to the floor-hash property (neither checks the floor hash). So the harness first **asserts the real cloned config carries the exact `floor-hash-verify` stanza wired to the guard script** (property 0), then **scopes the clone's config to that one real stanza** to prove it blocks a `git commit` (real hook definition + real guard + real commit-block). This is faithful to the #226 property — the unrelated hooks are scoped out *only* because they cannot resolve in a temp clone, **not** to weaken the test (R1 — clone-as-hub-sibling + network — was rejected: it adds fragility without additional floor-property proof). The harness also clones with `-c core.autocrlf=false` (LF checkout) so a system `autocrlf=true` cannot leave every LF-committed file reading as an unstaged modification (which pre-commit refuses to run over).
...
- **Greenfield `settings.json` tracking.** The SessionStart hook lives in `.claude/settings.json`, but the model-A `.gitignore` negation block tracks only the floor, sidecar, and guard script — **not** `settings.json`. This deferral is *principled*, not convenience: `settings.json` carries per-consumer/machine-specific content (e.g. an absolute marketplace path), so universal carrier-tracking would commit machine paths across the fleet. ai-council (the n=1 proving ground) already force-tracks its `settings.json`, so its self-arm holds. A *fully greenfield* consumer that gitignores `.claude/` with no pre-tracked `settings.json` would not carry the SessionStart hook on the next fresh clone → its session-start leg would not self-arm. This is now a **tested** property, not an assumption: `#230`'s `assert_sessionstart_wired` reads the (cloned) `settings.json` and asserts the SessionStart wiring is present — so a green Layer-2 against real ai-council *proves* self-arm, and a consumer whose `settings.json` did not travel *fails* the harness. Fleet rollout (#221) must extend the negation block to `!.claude/settings.json` or force-track it per consumer (splitting the machine-specific bits out first).
- **`--no-verify` and out-of-CC commits.** A `git commit --no-verify`, or a raw `git commit` in a clone never opened in CC, bypasses the commit-time leg. The session-start leg is the backstop (it catches the drift — or the delete — on the next session). Accepted per the accidental-drift threat model.
