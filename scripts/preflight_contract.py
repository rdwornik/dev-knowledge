...
_FILE_LINE_RE = re.compile(r"`([A-Za-z0-9_./\\-]+\.[A-Za-z0-9]{1,6}):(\d+)`")
...
def _resolve(repo_root: Path, rel: str) -> Path | None:
    """The file `rel` names, tried at the repo root then the conventional source roots."""
    for root in _SOURCE_ROOTS:
        candidate = repo_root / root / rel if root else repo_root / rel
...
def verify(contract: Path, repo_root: Path = _REPO_ROOT) -> Report:
    ...
    for m in _FILE_LINE_RE.finditer(text):
        ...
        raw = f"{rel}:{line}"
        target = _resolve(repo_root, rel)
        if target is None:
            add("file-line", raw, False, f"{rel} does not exist")
            continue
    ...
    for m in _SHA_RE.finditer(text):
        ...
        add("sha", sha, r.returncode == 0, "" if r.returncode == 0 else "not reachable in history")
