...
THRESHOLD_PROVENANCE: dict[str, str] = {
    ...
    "RSS_PLATEAU_MB": (
        "NOT PROPERLY DERIVED, AND THIS ENTRY IS THE ADMISSION. The frozen contract directs "
        "that the RSS threshold be computed from transcripts. It cannot be: no transcript "
        "record type carries any memory field (all 15 types enumerated), and the prescribed "
        "proxy -- per-turn latency against own median -- is FLAT from 0-15 min to 480+ min "
        "across 92,743 turns, so there is no rise to locate a threshold at. 385 MB is instead "
        "the CROSS-SECTIONAL plateau of a single 2026-09-15 snapshot of 31 different "
        "processes doing different work. A snapshot cannot distinguish 'processes grow' from "
        "'older processes are a different population'. `sample` exists to replace this figure "
        "with a longitudinal one; until it has run for a while, this number is a working "
        "bound and not a measurement of the thing it names."
    ),
    "MERGE_COUNT_BOUND": (
