...
(`ecosystem/harness.yaml`, moment `lane-end`, precondition). This guard makes the trigger event-driven:
  ...
  * HANDBACK LINE     -> take a once-only claim (`MOMENT-LANE-END-HOOK.json`, status `running`, under the
    lane's receipts dir), start ONE detached worker and return. The worker runs `doit moment:lane-end`
...
def spawn_worker(argv: list[str], cwd: Path, env: Mapping[str, str]) -> None:
    """Start `argv` detached from this hook (its stdio closed, its own group, outside the hook's job)."""
    ...
    except OSError:   # the hook's job forbids breakaway: start inside it rather than not at all
...
def _claim(path: Path, receipt: HookReceipt) -> bool:
    """Take the once-only claim. With no receipt yet the create is exclusive (two racing turn ends: one wins)."""
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
...
def _receipt(lane: str, status: str, exit_code: int, handback: Optional[str], reason: str,
             guard_ms: int, moment_ms: int = 0) -> HookReceipt:
    return HookReceipt(lane=lane, status=status, exit_code=exit_code, handback=handback, reason=reason,
                       ...
                       within_hook_limit=guard_ms < HOOK_LIMIT_S * 1000, finished_at=_stamp())
...
def _finish(receipt_path: Path, claim: dict, lane: str, run: Callable[[], MomentResult]) -> None:
    """Run the moment and replace the claim with the final receipt. Never raises."""
...
def main(argv: Optional[list[str]] = None, *, environ: Mapping[str, str] = os.environ,
    ...
    try:
        ...
        if handback is None or _read_receipt(receipt_path).get("handback") == handback:
            return 0
        claim_receipt = _receipt(lane, "running", EXIT_FAILED, handback, "the moment is running", _ms())
        try:
            if receipt_path.exists():
                _write_atomic(receipt_path, claim_receipt)   # a previous receipt, for another closing line
            elif not _claim(receipt_path, claim_receipt):
                return 0                                     # a concurrent turn end took it
        except OSError:
            return 0
        ...
        if not detach:
            ...
            return 0
        worker_argv = [sys.executable, str(Path(__file__).resolve()), "--worker"]
        if moment_argv:
            worker_argv += ["--moment-json", json.dumps(moment_argv)]
        try:
            (spawner or spawn_worker)(worker_argv, root, child_env)
