...
#: The model each seat BOOTS ON (operator ruling 2026-09-11, AMEND-BATCH-X-ROSTER-022 AX22-3).
#: Distinct from the Ch8 routing matrix, which routes an ARC's work: this is the tier the seat
#: itself runs at. The two seats named were the largest Opus sinks -- the dispatcher's work is
#: mechanical (freeze, DryRun, fire, receipts) and the integrator judges merge verdicts on Opus
#: while running suites and teardowns on Sonnet, which is what `opusplan` is. The remaining
#: three are unchanged, and are stated rather than defaulted so that adding a seat to the enum
#: without deciding its tier fails a test instead of rendering an unresolved one.
...
def _provenance(seat: str, batch: str, date: str, ch8_sha: str) -> str:
    ...
    return (
        ...
        f"     seat: {seat} | batch: {batch} | date: {date} | model: {SEAT_MODELS[seat]}"
