...
3. Read `templates/handoff/` (README + `01`–`07`). For each, resolve markers from
   ...
   - `{{CONTEXT: <var>}}` — substitute captured values (slug, repo, type, date,
     ...
     status). `version` = `4.3.1`; `status` = `stable` (promoted 2026-05-30 per
     v4.3.1 amendment item E — judgment-augmented criterion: <2 critical findings AND
     reviewer Stage-3 verdict PROMOTE/PROMOTE-WITH-CAVEATS; reviewer judgment
     overrides count).
...
3a. **Always-emit rules** (v4.2 items B/C/D + v4.3 item B — emit whether or not drift was found):
   ...
   - `README.md` header carries the stamp `Generated by HANDOFF_PROCESS v4.3.1 (status: stable)`.
