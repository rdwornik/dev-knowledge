...
3. Read `templates/handoff/` (README + `01`–`07`). For each, resolve markers from
   ...
   - `{{CONTEXT: <var>}}` — substitute captured values (slug, repo, type, date,
     ...
     status). `version` = `4.2`; `status` = `beta` (first three end-to-end runs of a
     version; `stable` thereafter — v4.2 item D).
...
3a. **v4.2 always-emit rules** (items B/C/D — emit whether or not drift was found):
   - `04_RECENT.md` Load-bearing facts table is a **required section**, columns fixed
     `Claim from sender | Repo fact | Verdict | Verification command`. No drift → the
   ...
   - `README.md` header carries the stamp `Generated by HANDOFF_PROCESS v4.2 (status: beta)`.
