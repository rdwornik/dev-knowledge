...
# --- #524 leg (a): whole-file JOURNAL day-letter check ----------------------------------

def test_journal_day_letters_duplicate_is_health_fail(tmp_path: Path, monkeypatch) -> None:
    """A seeded post-floor duplicate day-letter -> FAIL, naming date + letter."""
    (tmp_path / "JOURNAL.md").write_text(
        "### 2026-08-14 (a) — second entry (re-lettering missed)\ncontent\n\n"
        "### 2026-08-14 (a) — first entry\ncontent\n", encoding="utf-8")
    monkeypatch.setattr(aud, "_REPO_ROOT", str(tmp_path))
    f = aud.check_journal_day_letters(tmp_path)[0]
    assert f.status == "fail"
    assert "2026-08-14" in f.evidence


def test_journal_day_letters_contiguous_run_passes(tmp_path: Path, monkeypatch) -> None:
    """A clean, non-colliding post-floor letter run -> pass."""
    (tmp_path / "JOURNAL.md").write_text(
        "### 2026-08-14 (c) — third\ncontent\n\n"
        "### 2026-08-14 (b) — second\ncontent\n\n"
        "### 2026-08-14 (a) — first\ncontent\n", encoding="utf-8")
...
def test_journal_day_letters_pre_floor_collisions_are_exempt(tmp_path: Path, monkeypatch) -> None:
    """A duplicate dated BEFORE the doctrine's own ratification floor is exempt — this repo's
    live JOURNAL.md carries two such immutable pre-floor collisions (2026-07-26, 2026-05-09)
    that predate the letter-allocation-at-integration convention and are nobody's to rewrite."""
    (tmp_path / "JOURNAL.md").write_text(
        "### 2026-08-14 (a) — clean post-floor entry\ncontent\n\n"
        "### 2026-07-26 (g) — pre-floor duplicate 2\ncontent\n\n"
        "### 2026-07-26 (g) — pre-floor duplicate 1\ncontent\n", encoding="utf-8")
    monkeypatch.setattr(aud, "_REPO_ROOT", str(tmp_path))
    f = aud.check_journal_day_letters(tmp_path)[0]
    assert f.status == "pass"
...
def test_import_edges_live_repo_passes_and_is_registered() -> None:
    ...
    assert len(aud.ALL_CHECKS) == 56  # 55 -> 56: check_organ_truth added (lane-l3-organ-truth, 2026-09-20); 30 -> 31: check_residual_completeness added (ARC-5 residual gate, 2026-07-19); 29 -> 30: check_fleet_parity added ([#337], 2026-07-18); 31 -> 32: check_routine_consumers added ([#419]/ADR-105 activation gate, 2026-07-26); 32 -> 34: check_silent_rule_ratchet ([#436] D4 ratchet) + check_task_tree_coherence ([#433] C1 gate-arm) added, 2026-07-27; 34 -> 35: check_boot_byte_budget added ([#446] A10 item 2 / R4 boot byte budget, 2026-07-31); 35 -> 36: check_intake_tree_coherence added ([#383] wave 1 — ADR-109 §4 generality proof, 2026-07-31); 36 -> 37: check_fleet_audit_replication added ([#460] — ADR-80 durable-record replication alarm, 2026-08-01); 37 -> 38: check_membership_agreement added ([#462] — ADR-104 declaration vs every repo-keyed machine surface, 2026-08-01); 38 -> 39: check_journal_spine_anchor added (ADR-85 amendment 2026-08-03 §A8/FR4 — pre-push backstop, 2026-08-03); 39 -> 38: check_handoff_tag_canonicity RETIRED ([#465] leg 4 — its subject (§3.1 four-tag section) has not existed since the v5 flip; the inert-check detector found it, 2026-08-04); 38 -> 39: check_preflight_backlog_ids added ([#483] R3 -- ADVISORY leg, WARN-tier; hard-gating deferred pending 0 false positives over two windows, 2026-08-04); 39 -> 40: check_review_artifact_coverage added ([#480] P3 -- ADVISORY leg, WARN-tier; the hard pre-push leg is deferred pending 0 false positives over two consecutive windows, 2026-08-05); 40 -> 41: check_stale_worktrees added ([#505] batch hygiene -- WARN-tier by ruling, ADR-110 §1 item 4 / intake #26 Track 1 item 4, 2026-08-06); 41 -> 42: check_landing_predicate added ([#513] propagation completeness / landing-predicate scanner, 2026-08-13); 42 -> 43: check_journal_day_letters added ([#524] leg a — whole-file JOURNAL day-letter uniqueness since 2026-07-30, 2026-08-14); 43 -> 44: check_adr_status_grammar added ([#242] lane L3 — ADR Status grammar/enum + header<->README coherence; enum/single-field FAIL-armed, five legs WARN against a recorded baseline, 2026-08-24); 44 -> 46: check_generated_artifact_freshness (ADR-86 amd. 2026-08-23 / [#171] leg 1, WARN-tier by ruling) and check_funnel_coverage (M3 ADVISORY leg, WARN-tier by ruling; zero-baseline identity ratchet over docs/audits/ disposition coverage) added together by the mini-integration, 2026-08-24; 46 -> 47: check_supplement_folded added (R4, handoff census 2026-08-26 b6 — FAIL-class; a filled SUPPLEMENT that never reached its PASTE_THIS.md is a silent loss of the outgoing seat's judgment, measured at 1 live instance); 47 -> 48: check_dispatch_verb_agreement added (R5, handoff census 2026-08-26 b2 — FAIL-class; the drift organ STANDING_RULINGS §V records as "owed and unbuilt", tree-side half: lane-boot + prompt-template name the Ch8-ruled verb and no rival form)50  # 30 -> 31: check_residual_completeness added (ARC-5 residual gate, 2026-07-19); 29 -> 30: check_fleet_parity added ([#337], 2026-07-18); 31 -> 32: check_routine_consumers added ([#419]/ADR-105 activation gate, 2026-07-26); 32 -> 34: check_silent_rule_ratchet ([#436] D4 ratchet) + check_task_tree_coherence ([#433] C1 gate-arm) added, 2026-07-27; 34 -> 35: check_boot_byte_budget added ([#446] A10 item 2 / R4 boot byte budget, 2026-07-31); 35 -> 36: check_intake_tree_coherence added ([#383] wave 1 — ADR-109 §4 generality proof, 2026-07-31); 36 -> 37: check_fleet_audit_replication added ([#460] — ADR-80 durable-record replication alarm, 2026-08-01); 37 -> 38: check_membership_agreement added ([#462] — ADR-104 declaration vs every repo-keyed machine surface, 2026-08-01); 38 -> 39: check_journal_spine_anchor added (ADR-85 amendment 2026-08-03 §A8/FR4 — pre-push backstop, 2026-08-03); 39 -> 38: check_handoff_tag_canonicity RETIRED ([#465] leg 4 — its subject (§3.1 four-tag section) has not existed since the v5 flip; the inert-check detector found it, 2026-08-04); 38 -> 39: check_preflight_backlog_ids added ([#483] R3 -- ADVISORY leg, WARN-tier; hard-gating deferred pending 0 false positives over two windows, 2026-08-04); 39 -> 40: check_review_artifact_coverage added ([#480] P3 -- ADVISORY leg, WARN-tier; the hard pre-push leg is deferred pending 0 false positives over two consecutive windows, 2026-08-05); 40 -> 41: check_stale_worktrees added ([#505] batch hygiene -- WARN-tier by ruling, ADR-110 §1 item 4 / intake #26 Track 1 item 4, 2026-08-06); 41 -> 42: check_landing_predicate added ([#513] propagation completeness / landing-predicate scanner, 2026-08-13); 42 -> 43: check_journal_day_letters added ([#524] leg a — whole-file JOURNAL day-letter uniqueness since 2026-07-30, 2026-08-14); 43 -> 44: check_adr_status_grammar added ([#242] lane L3 — ADR Status grammar/enum + header<->README coherence; enum/single-field FAIL-armed, five legs WARN against a recorded baseline, 2026-08-24); 44 -> 46: check_generated_artifact_freshness (ADR-86 amd. 2026-08-23 / [#171] leg 1, WARN-tier by ruling) and check_funnel_coverage (M3 ADVISORY leg, WARN-tier by ruling; zero-baseline identity ratchet over docs/audits/ disposition coverage) added together by the mini-integration, 2026-08-24; 48 -> 49: check_substrate_declaration added ([#591] substrate validator layer 2 — REFUSE legs FAIL-armed against a post-2026-08-27 corpus measuring 0, second-local-writer leg WARN by the row's own words, 2026-08-27); 49 -> 50: check_dispatch_drift added ([#592] — Ch8's literal dispatch commands vs Get-Command on the operator host + /lane-boot's ruled verb; machine-dependent by design, a shell-less host WARNs its tier and never reports a green-rendering status, 2026-08-27); 50 -> 51: check_consumer_at_landing added ([#595] — the subtraction mechanism; leg 1 (a landed docs/audits artifact declares its consumer) FAIL-armed against a post-2026-08-27 corpus measuring 0, leg 2 (identity-keyed consumption ratchet) WARN on the funnel_coverage ruling, 2026-08-27); 51 -> 52: check_proof_layer added ([#596] — family 3 at the PROOF layer: a proof whose firing is gated on the environment it polices; WARN-tier identity ratchet over 38 environment-conditional guards, both sweep exemplars detected, 2026-08-27); 52 -> 53: check_routing_agreement added ([#613] -- the L0 routing copy vs the authoritative in-repo ecosystem/routing-table.yaml, register ruling Z-G3 amendment A2; SHIP-tier by declaration because the live L0 diverges at arm time and only the operator can derive it, so a COMMIT-tier FAIL would wedge every commit on a file the repo may not write; verdict stays fail-on-divergence / warn-on-absent, never 'unavailable' (Z-G4), 2026-08-28); 53 -> 54: check_funnel_lifecycle added (FM-2 — the funnel's EXIT step: a terminal intake/ADR that never left its live home, and a post-cutoff row whose provenance does not resolve. SHIP-tier by declaration — the THIRD exception to the assignment rule, because leg a1 FAILs on live main by design (intake #19/#26 ruled CONSUMED 2026-08-29 and still at docs/intake/ depth 1) and clearing it means relocating a governed document, which a committing lane may not do; promote to TIER_COMMIT when leg a1 measures 0 on main, 2026-08-29); 54 -> 55: check_plugin_version_drift added (DECLARE-F-2026-09-06 F-3 -- carrier #2's SECOND drift surface: the durable per-consumer deployed_plugin_version record vs the hub tier1-lifecycle plugin.json spec, reported in BOTH directions (consumer behind / record ahead of its release). WARN-class with a ZERO baseline -- every repo's field ships null, so it is n/a fleet-wide on landing; the deploy runbook writes the first value, 2026-09-06)
...
def test_fleet_parity_registered_in_all_checks():
    ...
    assert len(aud.ALL_CHECKS) == 56  # 55 -> 56: check_organ_truth added (lane-l3-organ-truth, 2026-09-20); ... 41 -> 42: check_landing_predicate added ([#513], 2026-08-13); 42 -> 43: check_journal_day_letters added ([#524] leg a, 2026-08-14). Full history: see test_all_checks_count_is_pinned (tests/test_writer_integrity.py).; 44 -> 46: check_generated_artifact_freshness (ADR-86 amd. 2026-08-23 / [#171] leg 1, WARN-tier by ruling) and check_funnel_coverage (M3 ADVISORY leg, WARN-tier by ruling; zero-baseline identity ratchet over docs/audits/ disposition coverage) added together by the mini-integration, 2026-08-24; 46 -> 47: check_supplement_folded added (R4, handoff census 2026-08-26 b6 — FAIL-class; a filled SUPPLEMENT that never reached its PASTE_THIS.md is a silent loss of the outgoing seat's judgment, measured at 1 live instance); 47 -> 48: check_dispatch_verb_agreement added (R5, handoff census 2026-08-26 b2 — FAIL-class; the drift organ STANDING_RULINGS §V records as "owed and unbuilt", tree-side half: lane-boot + prompt-template name the Ch8-ruled verb and no rival form)50  # ... 41 -> 42: check_landing_predicate added ([#513], 2026-08-13); 42 -> 43: check_journal_day_letters added ([#524] leg a, 2026-08-14). Full history: see test_all_checks_count_is_pinned (tests/test_writer_integrity.py).; 44 -> 46: check_generated_artifact_freshness (ADR-86 amd. 2026-08-23 / [#171] leg 1, WARN-tier by ruling) and check_funnel_coverage (M3 ADVISORY leg, WARN-tier by ruling; zero-baseline identity ratchet over docs/audits/ disposition coverage) added together by the mini-integration, 2026-08-24; 48 -> 49: check_substrate_declaration added ([#591] substrate validator layer 2 — REFUSE legs FAIL-armed against a post-2026-08-27 corpus measuring 0, second-local-writer leg WARN by the row's own words, 2026-08-27); 49 -> 50: check_dispatch_drift added ([#592] — Ch8's literal dispatch commands vs Get-Command on the operator host + /lane-boot's ruled verb; machine-dependent by design, a shell-less host WARNs its tier and never reports a green-rendering status, 2026-08-27); 50 -> 51: check_consumer_at_landing added ([#595] — the subtraction mechanism; leg 1 (a landed docs/audits artifact declares its consumer) FAIL-armed against a post-2026-08-27 corpus measuring 0, leg 2 (identity-keyed consumption ratchet) WARN on the funnel_coverage ruling, 2026-08-27); 51 -> 52: check_proof_layer added ([#596] — family 3 at the PROOF layer: a proof whose firing is gated on the environment it polices; WARN-tier identity ratchet over 38 environment-conditional guards, both sweep exemplars detected, 2026-08-27); 52 -> 53: check_routing_agreement added ([#613] -- the L0 routing copy vs the authoritative in-repo ecosystem/routing-table.yaml, register ruling Z-G3 amendment A2; SHIP-tier by declaration because the live L0 diverges at arm time and only the operator can derive it, so a COMMIT-tier FAIL would wedge every commit on a file the repo may not write; verdict stays fail-on-divergence / warn-on-absent, never 'unavailable' (Z-G4), 2026-08-28); 53 -> 54: check_funnel_lifecycle added (FM-2 — the funnel's EXIT step: a terminal intake/ADR that never left its live home, and a post-cutoff row whose provenance does not resolve. SHIP-tier by declaration — the THIRD exception to the assignment rule, because leg a1 FAILs on live main by design (intake #19/#26 ruled CONSUMED 2026-08-29 and still at docs/intake/ depth 1) and clearing it means relocating a governed document, which a committing lane may not do; promote to TIER_COMMIT when leg a1 measures 0 on main, 2026-08-29); 54 -> 55: check_plugin_version_drift added (DECLARE-F-2026-09-06 F-3 -- carrier #2's SECOND drift surface: the durable per-consumer deployed_plugin_version record vs the hub tier1-lifecycle plugin.json spec, reported in BOTH directions (consumer behind / record ahead of its release). WARN-class with a ZERO baseline -- every repo's field ships null, so it is n/a fleet-wide on landing; the deploy runbook writes the first value, 2026-09-06)
