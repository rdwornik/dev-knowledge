...
def test_fleet_parity_registered_in_all_checks():
    ...
    assert len(aud.ALL_CHECKS) == 49  # ... 41 -> 42: check_landing_predicate added ([#513], 2026-08-13); 42 -> 43: check_journal_day_letters added ([#524] leg a, 2026-08-14). Full history: see test_all_checks_count_is_pinned (tests/test_writer_integrity.py).; 44 -> 46: check_generated_artifact_freshness (ADR-86 amd. 2026-08-23 / [#171] leg 1, WARN-tier by ruling) and check_funnel_coverage (M3 ADVISORY leg, WARN-tier by ruling; zero-baseline identity ratchet over docs/audits/ disposition coverage) added together by the mini-integration, 2026-08-24; 46 -> 47: check_substrate_declaration added ([#591] substrate validator layer 2 — REFUSE legs FAIL-armed against a post-2026-08-27 corpus measuring 0, second-local-writer leg WARN by the row's own words, 2026-08-27); 47 -> 48: check_dispatch_drift added ([#592] — Ch8's literal dispatch commands vs Get-Command on the operator host + /lane-boot's ruled verb; machine-dependent by design, a shell-less host WARNs its tier and never reports a green-rendering status, 2026-08-27); 48 -> 49: check_consumer_at_landing added ([#595] — the subtraction mechanism; leg 1 (a landed docs/audits artifact declares its consumer) FAIL-armed against a post-2026-08-27 corpus measuring 0, leg 2 (identity-keyed consumption ratchet) WARN on the funnel_coverage ruling, 2026-08-27)
