...
RENAMED 2026-09-25 (LANE-5B-5 lane-hooks-port) from test_surface_triage.py to
test_surface_triage_ps1.py: the .ps1 file is retired-in-place (no longer wired in
.claude/settings.json -- PowerShell was the harness's only hard break on a Linux/cloud
substrate; the live hook is now scripts/surface_triage.py, guarded by the file that took over
the test_surface_triage.py name). The .ps1's own behaviour is unchanged, so this guard keeps
guarding it unchanged too.
...
(`powershell -NoProfile -File ...`) with a fake `gh` first on PATH answering the auth /
issue-list / run-list surfacings on their happy path. Mirrors the subprocess "wire protocol"
half of test_block_immutable_edits.py. NOTE: this is no longer the hook's actual runtime (see
the RENAMED note above) -- it is a pure regression guard on the retired-but-kept .ps1 file.
"""
