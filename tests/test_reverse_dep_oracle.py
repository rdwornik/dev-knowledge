...
def test_position_points_at_name_not_keyword():
    ...
    assert d.line == 303            # 0-based (audit.py:304 is 1-based)
...
def test_finding_headline_resolves_with_provenance():
    ...
    assert res["definition"]["line"] == 304                  # 1-based output
    ...
    assert {"file": "scripts/audit.py", "line": 304} not in ans["reverse_dependents"]
    prov = ans["provenance"]
