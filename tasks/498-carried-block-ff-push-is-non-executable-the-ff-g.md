...
**CLOSED 2026-08-05.** Fixed TDD-first on `fix/498-hook-exec-bit`: **`e164b220`** RED (test-only — the structural assertion over every `language: script` entry, failing on `block-ff-push -> scripts/block_ff_push.py tracked 100644`), then **`90089f36`** the fix (`git update-index --chmod=+x`, mode 100644 => 100755; blob hash unchanged at `32bd3a89`, so no content moved). All four carried script targets now read 100755. Terra review `docs/audits/2026-08-05-codex-498-hook-exec-bit.md` — tally 0/0/0/0, the first artifact written in the [#480] canonical header shape. The structural guard is the durable half: it iterates whatever the carrier declares, so the NEXT carried script is covered with no edit.
