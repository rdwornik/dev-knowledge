...
- **ADR-115** — **`AGENTS.md` is the portable instruction layer**: ADR-53 Decision 2 (CLAUDE.md as the single agent-instruction file) is **superseded**, and the ADR-101 Tier-1 file class amended in the same act to admit it. Consequence for this map: §10's *"Narrating or managing AGENTS.md"* anti-pattern in `CLAUDE.md` is now false doctrine — it sits inside a hub-single-sourced Form-A region, so its correction needs a lockstep template act and is owned by `[#577]` — Accepted 2026-08-25.
- **ADR-114** — root `README.md` recreation — **PARKED** 2026-08-22, carried here so an absent verdict is not read as an absent decision.
