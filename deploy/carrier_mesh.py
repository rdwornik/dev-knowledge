...
(nothing deployed), ``PRESENT_CORRECT`` (all artifacts present + correct + committable + the Stop
hook wired), ``PRESENT_DRIFTED`` (some present, some missing/wrong/gitignored).
...
_GITIGNORE_NEGATIONS = ("!.claude/commands/", "!.claude/commands/override.md")
_GITIGNORE_BLOCK = (_GITIGNORE_BEGIN, *_GITIGNORE_IGNORES, *_GITIGNORE_NEGATIONS, _GITIGNORE_END)
...
def _gitignore_block_present(path: Path) -> bool:
    ...
    return all(x in lines for x in (*_GITIGNORE_IGNORES, *_GITIGNORE_NEGATIONS))
...
def _is_gitignored(root: Path, rel: str) -> bool:
    """True if `rel` is gitignored in the consumer (so `git add -A` would silently skip it).

    `git check-ignore -q` exits 0 when the path IS ignored, 1 when not. This is the
    committability check verify() must make: a written-but-ignored artifact passes a
    working-tree-presence check yet never reaches a commit (the clone the fire reads).
    """
    try:
...
def _ensure_gitignore(path: Path) -> str | None:
    ...
    if all(x in stripped for x in (*_GITIGNORE_IGNORES, *_GITIGNORE_NEGATIONS)):
        return None
    existed = path.exists()
    ...
    new = (body + "\n\n" + block + "\n") if body else (block + "\n")
    _write_lf(path, new)
    return f"{'updated' if existed else 'wrote'} {path.name} (enforcement-mesh block)"
