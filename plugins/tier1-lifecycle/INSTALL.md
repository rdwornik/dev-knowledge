...
|---|---|---|
| **Plugin source changed** in `.dev-knowledge` (you committed to `plugins/tier1-lifecycle/`) | **Bump `version` in `plugin.json`**, then `claude plugin marketplace update dev-knowledge-methodology` + `claude plugin install tier1-lifecycle@dev-knowledge-methodology --scope project` (per repo) | **The cache is keyed by version** (`~/.claude/plugins/cache/.../<version>/`). `marketplace update` alone does NOT refresh files at an unchanged version — installed repos keep running the old cached copy. A version bump creates a fresh `<new-version>/` cache dir. (verified 2026-06-02) |
