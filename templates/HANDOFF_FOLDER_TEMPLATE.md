...
- `next_session_scope` (declared at Stage 2; selects the operational layer — per ADR-57)
- File index with one-line purpose each (same 02_VISION / 02b conditional rules as 00_README.md), including `10_GATE_PROBE.md`, `11_CLAIMS.md`, and any `12_OPERATIONAL_*` artifacts
- Invariant integrity line: canonical hash/version IDs for VISION/PLAYBOOK/ESSENTIALS (per ADR-42 Q5 amendment)
...
invariant integrity tracking (per ADR-42 Q5 amendment).
...
```json
{
  ...
  "next_session_scope": "{code-implementation|architecture-decision|audit-work|documentation|mixed-uncertain}",
  "target_repo": {
    "name": "{repo}",
    "path": "{absolute path}",
  ...
  "invariant_integrity": {
    "02_VISION.md": {"canonical_sha256": "...", "version_id": "{frontmatter version or canonical commit SHA}"},
    "03_PLAYBOOK.md": {"canonical_sha256": "...", "version_id": "..."},
    "04_ESSENTIALS.md": {"canonical_sha256": "...", "version_id": "..."}
  },
  "files": [
    ...
    {"path": "10_GATE_PROBE.md", "sha256": "..."},
    {"path": "11_CLAIMS.md", "sha256": "..."},
    {"path": "12_OPERATIONAL_*", "sha256": "...", "conditional": "one entry per operational-layer artifact selected by next_session_scope"}
  ]
}
```

`invariant_integrity` carries the canonical hash + version ID for each governance-floor
file. The executor-side validator (contract per ADR-42 Q5 amendment; wiring deferred to
BACKLOG) fails on a mismatch between the bundle copy and the canonical `.dev-knowledge`
source when that source is accessible — guarding bundle/canonical drift while full
copies are still delivered.
