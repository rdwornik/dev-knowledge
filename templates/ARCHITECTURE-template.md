...
**Governing conventions** (these live in the `.dev-knowledge` ecosystem repo, not in this repo): ADR-51 (architecture doc convention, including amendment 2026-05-22 — codemap generator output specification), ADR-38 (universal repo baseline; root-placement amendment A3), ADR-36 (read-only audit contract).
...
The codemap is the canonical artifact answering *"what exists and how does it relate?"*. ADR-51 Decision 6 + amendment 2026-05-22 specify its form:

- **Default — graphical**: an auto-generated embedded Mermaid block between `<!-- CODEMAP:START -->` and `<!-- CODEMAP:END -->` markers. Generated by `python -m scripts.codemap.cli generate . --write` (see `protocols/PLAYBOOK.md` § Codemap workflow). A pre-commit freshness hook (`codemap check`) blocks commits if this block drifts from the current source state.
- **Transitional — text-only**: a repo too small for a meaningful package graph MAY use a text-only module overview (no diagram, no generator dependency) until a graph is worthwhile. Operator judgment, no longer tier-gated. See the text-only override below.
...
**Canonical target (graphical default):**
...
flowchart TD
    ...
    click schema href "src/schema/" "Open schema"
```
...
> **Text-only override.** A repo too small for a meaningful package graph MAY replace the CODEMAP-bounded block above with a plain prose paragraph listing the modules and their roles. No CODEMAP markers, no diagram, no generator dependency. This is an operator judgment for very small repos, not a tier rule.
...
> The **module-map / codemap** diagram is **not** listed here — it lives in §2 Codemap (the canonical graphical codemap). Adding a duplicate "module map" diagram in this section creates two competing maps that drift independently.
