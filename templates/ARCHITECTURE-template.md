...
> This block is auto-generated by `python -m scripts.codemap.cli generate . --write` (see the hub methodology protocol `PLAYBOOK.md` § Codemap workflow — at the hub `.dev-knowledge/protocols/` set; hub-pointer, never copied into a consumer). The pre-commit hook (`codemap check`) blocks commits if this block drifts from current source state. To opt in to generator-based maintenance, see ADR-51 amendment 2026-05-22 § Per-repo adoption — opt-in, not mandatory.
